...
try
{
	...
	//查询当月是否封帐
	memset(sql,0,sizeof(sql));
	sprintf(sql,"select count(*) from D_CHECK_FILE_DETAIL where source_id = '%s' and check_type = 'AUD' and rate_cycle = '%s' and cycle_flag = '1'",Smonth.szSourceId,tmp_month);
	stmt.setSQLString(sql);
	stmt.execute();
	stmt>>ret;
	if(ret == 0)
	...
	string ft;
	memset(sql,0,sizeof(sql));
	sprintf(sql,"select distinct(file_time) from D_CHECK_FILE_DETAIL where source_id = '%s' and check_type = 'AUD' and rate_cycle = '%s'  order by  file_time desc ",Smonth.szSourceId,tmp_month);
	stmt.setSQLString(sql);
	stmt.execute();
	while(stmt>>ft)
	{
	...
	memset(sql,0,sizeof(sql));
	sprintf(sql,"select count(*) from D_SUMMARY_RESULT where sourceid = '%s' and sumtype in('D','RD') and sumdate = :1 ",Smonth.szSourceId);
