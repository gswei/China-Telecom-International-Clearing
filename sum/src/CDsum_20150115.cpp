...
int CDsum::sum(SDayList &Sday,char *sumday)
{
	...
	long cdr_cnt = 0,cdr_duration = 0,cdr_dataflow=0,totalFee = 0,feeTmp = 0,deal_flag=0;
		...
		{	
			...
			if(deal_flag == 0)	//非长途业务或者长途业务单账期时
			{
				...
				if(iter != monthSumMap.end())
				{
					...
					memset(sql,0,sizeof(sql));
					sprintf(sql,"select sum(%s),sum(%s),sum(%s)",iter->second.cdr_count,iter->second.cdr_duration,iter->second.cdr_dataflow);
					vector<string> vfee = iter->second.cdr_fee;
					for(int i = 0;i<vfee.size();i++)
					{
							sprintf(sql,"%s,sum(%s)",sql,vfee[i]);
					}
					sprintf(sql,"%s from %s_%s where %s=%s ",sql,iter->second.sum_table,vrc[i],time_col,sumday);

					theJSLog<<"(张 分 钱) sql = "<<sql<<endi;

					stmt.setSQLString(sql);
					stmt.execute();
					stmt>>cdr_cnt>>cdr_duration>>cdr_dataflow;
					...
					memset(sql,0,sizeof(sql));
					sprintf(sql,"insert into d_daysum_log(rate_cycle,source_id,curr_call_times,curr_duration,curr_fee,curr_dataflow,data_flag,accum_date,stat_time)"
								 " values (%s,'%s',%ld,%ld,%ld,%ld,2,%s,%s)",vrc[i],Sday.szSourceId,cdr_cnt,cdr_duration,totalFee,cdr_dataflow,sumday,currTime);
					stmt.setSQLString(sql);
					stmt.execute();
				}
				else
				{
					memset(erro_msg,0,sizeof(erro_msg));
					sprintf(erro_msg,"checkMonthFile 配置表中没有找到月汇总文件文件类型为[%s]的信息",Sday.szSourceId);
		...
		if(deal_flag)	
		{
			...
			if(iter != monthSumMap.end())
			{
				...
				{ 
					...
					sprintf(sql,"%s from %s_%s where %s=%s and %s = '%s' ",sql,iter->second.sum_table,vrc_tmp[0],time_col,sumday,iter->second.rate_cycle,vrc_tmp[0]);
					...
					memset(sql,0,sizeof(sql));
					sprintf(sql,"insert into d_daysum_log(rate_cycle,source_id,curr_call_times,curr_duration,curr_fee,data_flag,accum_date,stat_time)"
								 " values (%s,'%s',%ld,%ld,%ld,2,%s,%s)",vrc_tmp[0],Sday.szSourceId,cdr_cnt,cdr_duration,totalFee,sumday,currTime);
					stmt.setSQLString(sql);
					stmt.execute();
					...
					sprintf(sql,"%s from %s_%s where %s=%s and %s = '%s' ",sql,iter->second.sum_table,vrc_tmp[0],time_col,sumday,iter->second.rate_cycle,vrc_tmp[1]);
					...
					memset(sql,0,sizeof(sql));
					sprintf(sql,"insert into d_daysum_log(rate_cycle,source_id,curr_call_times,curr_duration,curr_fee,data_flag,accum_date,stat_time)"
								 " values (%s,'%s',%ld,%ld,%ld,2,%s,%s)",vrc_tmp[1],Sday.szSourceId,cdr_cnt,cdr_duration,totalFee,sumday,currTime);
					stmt.setSQLString(sql);
					stmt.execute();
				}
				...
				{ 
					...
					sprintf(sql,"%s from %s_%s where %s=%s and %s = '%s' ",sql,iter->second.sum_table,vrc_tmp[1],time_col,sumday,iter->second.rate_cycle,vrc_tmp[1]);
				...
				{ 
					...
					if(deal_flag == 1)		//上月后补数据
					{
						...
						sprintf(sql,"%s from %s_%s where %s=%s and %s = '%s' ",sql,iter->second.sum_table,vrc_tmp[1],time_col,sumday,iter->second.rate_cycle,vrc_tmp[2]);
						...
						memset(sql,0,sizeof(sql));
						sprintf(sql,"insert into d_daysum_log(rate_cycle,source_id,curr_call_times,curr_duration,curr_fee,data_flag,accum_date,stat_time)"
									 " values (%s,'%s',%ld,%ld,%ld,2,%s,%s)",vrc_tmp[2],Sday.szSourceId,cdr_cnt,cdr_duration,totalFee,sumday,currTime);
						stmt.setSQLString(sql);
						stmt.execute();
					}
				}
