...
CShare::~CShare()
{
	if(pShareList != NULL)
	{
		...
		{
			SCom scc = pShareList[i].tableItem ;
			if(scc.count > 1)
			{
				if(scc.mscontion != NULL) delete[] scc.mscontion;
			}

			if(pShareList[i].vItemInfo != NULL) delete[] pShareList[i].vItemInfo;
		}
...
int CShare::loadSumConfig(char *source_id, char *source_group_id,int pos)
{
 ...
 try
 {
	...
	Statement stmt = conn.createStatement();
	memset(sql,0,sizeof(sql));
	sprintf(sql, "select MONSUMT,MONSUMT_TCOL,SETTSUMT,SETTSUMT_TCOL,CONDITION from C_SUMTABLE_DEFINE where sourceid = '%s'",source_id );
	stmt.setSQLString(sql);
	stmt.execute();

	SCom scom;
	char sconfig[100],scondition[512];
	vector<string> vconfig,vcondition;

	memset(sconfig,0,sizeof(sconfig));
	memset(scondition,0,sizeof(scondition));

	stmt>>scom.iOrgSumt>>scom.szOrgSumtCol>>sconfig>>scom.szDestSumtCol>>scondition;
	...
	//2013-12-16目标表可能有多个
	splitString(sconfig,";",vconfig,true,true);
	splitString(scondition,";",vcondition,true,true);
	scom.count = vconfig.size();
	if(scom.count == 0)
	{
		memset(erro_msg,0,sizeof(erro_msg));
		sprintf(erro_msg,"数据源[%s] 对应的摊分结果表config_id不能为空",source_id);
		theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);
		return -1;
	}
	else if(scom.count == 1)
	{
		pShareList[pos].vItemInfo = new vector<SItemPair>[1];

		scom.iDestSumt = atoi(vconfig[0].c_str());
		...
		ret = getItemInfo(scom,pShareList[pos].vItemInfo[0]);		//获取输入输出的统计字段的值
		if(ret)  return -1;

	}
	else	//多个configid
	{
		if(vconfig.size() != vcondition.size())
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"数据源[%s]的摊分汇总结果表configID配置的个数[%d]和条件个数[%d]不一致",source_id,vconfig.size(),vcondition.size());
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);		
		}
		
		theJSLog<<"数据源ID="<<source_id<<" 原表ID="<<scom.iOrgSumt<<" 原表表名="<<scom.iOrgTableName<<" 原始表时间字段"<<scom.szOrgSumtCol
				<<"  目标表时间字段"<<scom.szDestSumtCol<<endi;
				//<<" 目标表ID="<<scom.iDestSumt<<" 目标表表名="<<scom.iDestTableName<<"  目标表时间字段"<<scom.szDestSumtCol<<endi;

		scom.mscontion = new SettCon[scom.count];
		pShareList[pos].vItemInfo = new vector<SItemPair>[scom.count];

		for(int i = 0;i<scom.count;i++)
		{
			scom.iDestSumt = atoi(vconfig[i].c_str());
			...
			strcpy(scom.mscontion[i].iDestTableName,scom.iDestTableName);
			strcpy(scom.mscontion[i].condition,vcondition[i].c_str());
			scom.mscontion[i].conSumt = atoi(vconfig[i].c_str());
			
			theJSLog<<"["<<i+1<<"] 目标表ID="<<scom.mscontion[i].conSumt<<" 目标表表名="<<scom.mscontion[i].iDestTableName<<"   目标表条件="<<scom.mscontion[i].condition<<endi;	
						
			ret = getItemInfo(scom,pShareList[pos].vItemInfo[i]);		//获取输入输出的统计字段的值
			if(ret)  return -1;
		}
...
try
{
	Statement stmt = conn.createStatement();
	memset(sql,0,sizeof(sql));
	sprintf(sql,"select count(1) from D_SUMMARY_RESULT where sourceid = '%s' and sumtype = 'F' and sumdate = '%s' and deal_flag = 'Y' and deal_flag = 'Y' ",Smonth.szSourceId,tmp);
	//cout<<"sql = "<<sql<<endl;
	stmt.setSQLString(sql);
	stmt.execute();
	stmt>>ret;
	...
	//***********************************
	memset(sql,0,sizeof(sql));
	sprintf(sql,"select count(1) from D_SUMMARY_RESULT where sourceid = '%s' and sumtype in('M','RM') and sumdate = '%s' and deal_flag = 'Y'",Smonth.szSourceId,tmp);
	//cout<<"sql = "<<sql<<endl;
	stmt.setSQLString(sql);
	stmt.execute();
	stmt>>ret;
	...
	{
		memset(erro_msg,0,sizeof(erro_msg));
		sprintf(erro_msg,"数据源[%s]在账期[%s]还没有完成月汇总",Smonth.szSourceId,tmp);
		theJSLog<<erro_msg<<endw;

		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SUMMARY_RESULT(SOURCEID,SUMTYPE,SUMDATE,SUMCOUNT,DEALTIME,DEAL_FLAG,DEAL_DESC) values('%s','%s','%s',0,to_char(sysdate,'yyyymmddhh24miss'),'N','%s')",Smonth.szSourceId,"F",tmp,erro_msg); 
		stmt.setSQLString(sql);
		stmt.execute();
		stmt.close();
...
/*
		...
		for(int i = 0;i<Smonth.vItemInf.size();i++)		//查询表的统计项统计维度等
		{
	...
	try
	{
		...
		//{		
			...
			*/
			...
			{
				for(int i = 0;i<Smonth.tableItem.count;i++)
				{
					memset(sql,0,sizeof(sql));
					//sprintf(sql,"delete from %s_%s where %s = '%s'",Smonth.tableItem.mscontion[i].iDestTableName,month,time_col,month);
					sprintf(sql,"delete from %s_%s ",Smonth.tableItem.mscontion[i].iDestTableName,month);
					cout<<"["<<i+1<<"] 删除汇总表结果sql="<<sql<<endl;
...
/*
		...
		{	
			...
			else
			{
				...
				{
					memset(sql,0,sizeof(sql));
					sprintf(sql,"select count(1) from %s_%s where %s = '%s'",Smonth.tableItem.mscontion[i].iDestTableName,month,time_col,month);
					stmt.setSQLString(sql);
					stmt.execute();
					stmt>>last_cnt;
					cout<<"["<<i+1<<"]保留以前数据记录条数:"<<last_cnt<<endl;
				}
			}
		}
*/		
		...
		if(Smonth.tableItem.count ==  1)
		{
			ret = getSql(Smonth.tableItem,Smonth.vItemInfo[0],month,sql,3);			//拼接sql
			if(ret == -1)
			{		
				stmt.rollback();
				return -1 ;
		...
		else
		{
			...
			for(int i = 0;i<Smonth.tableItem.count;i++)
			{
				ret = getSql(Smonth.tableItem,Smonth.vItemInfo[i],month,sql,3,"",i);		//拼接sql
				if(ret == -1)
				{	
					stmt.rollback();
					return -1 ;
				...
				memset(sql,0,sizeof(sql));
				//sprintf(sql,"select count(1) from  %s_%s where %s = '%s'",Smonth.tableItem.mscontion[i].iDestTableName,month,time_col,month);
				sprintf(sql,"select count(1) from  %s_%s ",Smonth.tableItem.mscontion[i].iDestTableName,month);
				//cout<<"统计摊分汇总记录条数sql:"<<sql<<endl;
				stmt.setSQLString(sql);
				stmt.execute();
				stmt>>cnt_tmp;
				
				theJSLog<<"插入记录数:"<<cnt_tmp<<endi;
				cnt += cnt_tmp;
			}
		}
/*		
...
*/
		...
		theJSLog<<"总共新增记录条数:"<<(cnt-last_cnt)<<"	写汇总结果表D_SUMMARY_RESULT"<<endi;
		//写摊分汇总日志
		memset(sql,0,sizeof(sql));
