	...
	if (dbConnect(conn))
	 {
		 ...
		 //配置中主要是目标表的内容
	     sprintf(sqltmp,"select a.table_item,a.field_name,a.item_type,a.field_begin,a.field_end,a.DEFV_OR_FUNC from C_STAT_TABLE_FMT a where a.config_id = %d order by a.seq ",dconfig);
         stmt.setSQLString(sqltmp);	
	     stmt.execute(); 
		 SItemPair item ;
	  
	    while(stmt>>item.toItem.szItemName>>item.fromItem.szItemName>>item.toItem.iItemType>>item.toItem.iBeginPos>>item.toItem.iEndPos>>item.toItem.idefault)
		{
  ...
  for(int i = 0;i<vItemInfo.size();i++)			//查询表的统计项统计维度等
  {
	...
	switch(toItem.iItemType)					//区分统计项和统计维度，是为了保证原表和目标表字段对齐
	{
		...
		case 12:					//标志取当前时间
				...
				break;
		case 13:					//2014-01-09 表示取默认值
				sqlB2.append(toItem.szItemName).append(",");
				sqlA2.append(toItem.idefault).append(",");
				break;
		default :
				break ;
   }
