...
try
{
	...
	sprintf(sql,"select count(*) from d_sch_format where SOURCE_ID like '%s%s' and file_time = '%s' and deal_flag  = 'E' union " 
	...
	sprintf(sql,"select count(*) from d_sch_wrtf where SOURCE_ID like '%s%s' and file_time = '%s' and deal_flag  = 'E' ",Sday.szSourceId,"%",sumday);
	stmt.setSQLString(sql);
	stmt.execute();
	stmt>>out_sum2;
...
int CDsum::sum(SDayList &Sday,char *sumday)
{
	...
	{	
		...
		string rc;
		memset(sql,0,sizeof(sql));
		sprintf(sql,"select distinct(rate_cycle) from D_CHECK_FILE_DETAIL where source_id like '%s%s' and check_type = 'AUD' and file_time = '%s' ",Sday.szSourceId,"%",sumday);
		cout<<"sql = "<<sql<<endl;
		stmt.setSQLString(sql);
		stmt.execute();
		while(stmt>>rc)
