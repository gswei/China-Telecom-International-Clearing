 ...
 {	
	...
	{	
		...
		if(mdrParam.drStatus == 1)		//主备系统
		{
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrParam.m_SerialString,"|",data,false,false);  //发送的字符串: file_id|file_id|
			...
			{	
				...
				{	
					...
					for(int i = 0;i<data.size();i++)		//查询每个文件
					{
						...
						if(!flag)	//有文件没有找到,重新找
						{
							theJSLog<<"文件file_id:"<<data[i]<<"没找到"<<cnt<<endi;
							sleep(10);
		...
		else
		{
			...
			while(stmt>>errinfo.source_id>>errinfo.filename>>errinfo.err_msg>>errinfo.err_code>>errinfo.err_col>>errinfo.err_line>>errinfo.err_seq)
			{
				...
				if(iter == erroinfoMap.end())
				{	
					if(erroinfoMap.size() >= 20)  break;  //新找到文件,防止仲裁信息超出4096

					vector<ERRINFO> errinfoV; 
					errinfoV.push_back(errinfo);
					erroinfoMap.insert(map< string,vector<ERRINFO> >::value_type(errinfo.filename,errinfoV));
					
					snprintf(mdrParam.m_SerialString,sizeof(mdrParam.m_SerialString),"%s%ld|",mdrParam.m_SerialString,errinfo.err_seq); //每次新添加一个文件时,因为记录级错误可能存在多条记录				
				}
				else
				{
					(iter->second).push_back(errinfo);
		...
		for(map< string,vector<ERRINFO> >::const_iterator iter= erroinfoMap.begin();iter != erroinfoMap.end();++iter)
		{
				...
				snprintf(mdrParam.m_AuditMsg,sizeof(mdrParam.m_AuditMsg),"%s%s,%s,%ld:",mdrParam.m_AuditMsg,vv[0].source_id,vv[0].err_msg,vv[0].err_seq);//文件名,数据源,文件错误类型
				...
				if( it == m_SourceCfg.end())
				{
					...
					snprintf(mdrParam.m_AuditMsg,sizeof(mdrParam.m_AuditMsg),"%s%s not find;",mdrParam.m_AuditMsg,vv[0].source_id);
					continue;
				...
				{
					sprintf(mdrParam.m_AuditMsg,"%s%s,%d,%d;",mdrParam.m_AuditMsg,vv[i].err_code,vv[i].err_line,vv[i].err_col); //错误代码,错误行,错误列
				...
				snprintf(mdrParam.m_AuditMsg,sizeof(mdrParam.m_AuditMsg),"%s|",mdrParam.m_AuditMsg);
...
int main(int argc,char** argv)
{
	...
	cout<<"*    last update time :  2013-12-02 by  hed	 "<<endl;
	cout<<"********************************************* "<<endl;
