...
void CSql::run()//扫描目录，获取文件名
{
	...
	try
	{
		if(mdrParam.drStatus==1)  //备系统
		{
				...
				splitString(mdrParam.m_SerialString,";",mVfile,false,false);
		...
		else      //主系统,非容灾系统    
		{
			...
			if(mVfile.size() != 0)
			{
				...
				for(int i = 0;i<mVfile.size();i++)
				{
					sprintf(mdrParam.m_SerialString,"%s%s;",mdrParam.m_SerialString,mVfile[i]);
				}
...
{	
	...
	for(int i = 0;i<mVfile.size();i++)
	{
		...
		try{			
				...
				theJSLog<<"处理sql文件:"<<m_Filename<<endi;
				...
				if(!in)
				{
					...
					sprintf(mdrParam.m_AuditMsg,"%s%s|",mdrParam.m_AuditMsg,strerror(errno));
				...
				sprintf(mdrParam.m_AuditMsg,"%s%c;%d|",mdrParam.m_AuditMsg,flag,count);
		  }
		  catch( SQLException e ) 
		  {
				stmt.rollback();
		...
		if(ret)										//仲裁失败
		{
			...
			{
				if(mdrParam.drStatus != 1)			//只考虑主系统情况
				{
...
int main(int argc,char** argv)
{
	...
	cout<<"*     last update time : 2013-12-03 by  cwf	* "<<endl;
	cout<<"********************************************** "<<endl;
