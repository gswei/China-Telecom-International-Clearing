...
C_BillStat::C_BillStat( char *_id,int index )
{

	Init(_id,index);
}
int C_BillStat::Init(char *_id,int index)
{
    ...
    if(max_count == 0)
    {
	   ...
	   p_stat = new S_BillStat[ MAX_BILL_STAT ];
   	}
...
//   Month[1][4] = '1';
//   Month[1][5] = '2';
   sprintf( Month[1],"%04u12", nowtimer.tm_year+1900-1);
...
int C_BillStat::check_time(char *buf)
{
    int i,len;
    len = strlen( buf );
    for (i = 0;i < len;i++)
...
int C_BillStat::AddItem( char *_time )
{
    if( p_stat == NULL )
        return -1;
    int i,found = 0;
    ...
    if (!found)
    {
        ...
        {
            S_BillStat *tmp = new S_BillStat[max_count + MAX_BILL_STAT];
...
//int C_BillStat::UpdateDB( char *m_log,CDatabase& DBConn)
int C_BillStat::UpdateDB( char *m_log,DBConnection conn)
{
    int i,j;
    if( p_stat == NULL ){
...
//        ds.Open("UPDATE D_BILL_STATICS SET BILL_COUNT = BILL_COUNT+:billnum2 "
...
//            ds.Close();
//            ds.Open("insert into D_BILL_STATICS VALUES( :source_id,:proc_index,:time,:billnum2 )",NONSELECT_DML);
...
//            }
		...
		//sql = "select count(*) from D_BILL_STATICS WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time"	
	    //stmt.setSQLString(sql);
		//stmt<<_source_id<<iIndex<<_time;
		//stmt.execute();
		//stmt>>count;
		sql = "UPDATE D_BILL_STATICS SET BILL_COUNT = BILL_COUNT+:billnum2 "
					 "WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time";	
		stmt.setSQLString(sql);
		stmt<<_billnum2<<_source_id<<iIndex<<_time;		

		if(!stmt.execute())  //insert, update, delete语句影响行数为0
		{			
			sql = "insert into D_BILL_STATICS VALUES( :source_id,:proc_index,:time,:billnum2 ";
			stmt.setSQLString(sql);
...
void C_BillStat::DelSpace( char *ss )
{
    int i,len,j;
    i = strlen(ss)-1;
    while (i && ss[i] == ' ') i--;
...
//int C_BillStat::DelUpdateDB( char *m_log,CDatabase& DBConn)
int C_BillStat::DelUpdateDB( char *m_log,DBConnection conn)
{
    int i,j;
    if( p_stat == NULL ){
    	sprintf( m_log,"the pointer p_stat is NULL!!" );
...
//        ds.Open("SELECT BILL_COUNT FROM D_BILL_STATICS  "
//        "WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time");
//		ds<<_source_id<<iIndex<<_time;
//		ds>>_billnum;
//      ds.Close();
		sql = "SELECT BILL_COUNT FROM D_BILL_STATICS  "
			 "WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time";
		Statement stmt  = conn.createStatement();
		stmt.setSQLString(sql);
		stmt<<_source_id<<iIndex<<_time;
		...
		if(!stmt.execute())
        {
            sprintf(m_log,"SELECT NONE FROM D_BILL_STATICS WHERE SOURCE_ID = '%s' and proc_index = %d and STATICS_TIME = '%s'",_source_id,iIndex,_time);
            return -1;
        }
        else
        {//update
          ...
          {
            sprintf(m_log,"Bill_count from D_BILL_STATICS where source_id = '%s'  and proc_index = %d and statics_time = '%s' is %d , Less than %d from file",_source_id,iIndex,_time,_billnum,_billnum2);
            return -1;
          }
          if(_billnum == _billnum2)
          {
//            ds.Open("delete D_BILL_STATICS WHERE SOURCE_ID = :source_id  and proc_index = :proc_index and STATICS_TIME = :time",NONSELECT_DML);
// 			ds<<_source_id<<iIndex<<_time;
//            ds.Execute();    
//            ds.Close();
			 sql = "delete D_BILL_STATICS WHERE SOURCE_ID = :source_id  and proc_index = :proc_index and STATICS_TIME = :time";
			 stmt.setSQLString(sql);
			 stmt<<_source_id<<iIndex<<_time;
			 stmt.execute();
			 stmt.close();
           ...
           {
            _billnum -= _billnum2;
//            ds.Open("UPDATE D_BILL_STATICS SET BILL_COUNT = :billnum2 "
//	        "WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time",NONSELECT_DML);
//			ds<<_billnum<<_source_id<<iIndex<<_time;
//            ds.Execute();
//            ds.Close();
			 sql = "UPDATE D_BILL_STATICS SET BILL_COUNT = :billnum2 "
					"WHERE SOURCE_ID = :source_id and proc_index = :proc_index and STATICS_TIME = :time";
			 stmt.setSQLString(sql);
