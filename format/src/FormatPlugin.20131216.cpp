...
{	
	...
	if(mdrParam.drStatus == 1)  //备系统接收到内容后才进行处理
	{
		...
		//获取同步变量
		vector<string> data;		
		splitString(mdrParam.m_SerialString,";",data,false,false);  //发送的字符串数据源ID,文件名,sqlFile
...
*/
		...
		{	
			...
			if(cycle_flag == 'Y')			//封帐文件
			{
				...
				sprintf(erro_msg,"[%s]封帐文件标志,F120",m_szFileName);					
				theJSLog.writeLog(LOG_CODE_FILE_NAME_REPEAT ,erro_msg);	
