...
int FormatPlugin::InsTimeBetweenFile(char *SourceId,char *filename,char *EarlyTime,char *LastTime)
{
  ...
  {
	  cout<<"InsTimeBetweenFile时操作数据库失败"<<e.what()<<endl;
...
int FormatPlugin::getFromSrcEnv( char *Value, char *Name, char *SourceId,char *szService)
{
  ...
  {
		cout<<"getFromSrcEnv抛出异常: "<<e.what()<<endl;
		//stmt.close();
		//conn.close();
...
//source
    ...
    //ds<<m_szSrcGrpID;
	sql = "select a.source_id,b.file_fmt,b.source_path,b.TOLLCODE from C_SOURCE_GROUP_CONFIG a,I_SOURCE_DEFINE b where SOURCE_GROUP=:1 and a.source_id=b.source_id";
    stmt.setSQLString(sql);
	stmt<<m_szSrcGrpID;
	if(stmt.execute())
	{
    ...
    for (int i=0; i<iSourceCount; i++)
    {
	  ...
	  stmt>>SourceCfg.szSourceId>>SourceCfg.szInFileFmt>>SourceCfg.szSourcePath>>SourceCfg.szTollCode;
...
int FormatPlugin::getFileNameFmt(string source)
{
	...
	try
	{	Statement stmt = conn.createStatement();
	    sql = "select check_no,check_index,check_len,check_type,check_value from C_FILENAME_CHECK where source_id = :1 order by check_no ";		
		stmt.setSQLString(sql);
		stmt << source;
		if(!stmt.execute())
		{
		...
		FileNameFmt fmt;
		while(stmt>>fmt.number>>fmt.index>>fmt.len>>fmt.check_type>>fmt.check_value)
		{
			vfmt.push_back(fmt);
			count++;
		}
	...
	{
		//cout<<"getFileNameFmt 数据库查询异常"<<e.what()<<endl;
		memset(erro_msg,0,sizeof(erro_msg));
...
int FormatPlugin::getRecordHTFilter(char* fileType)
{
	...
	try
	{	Statement stmt = conn.createStatement();
		sql = "select col_no,col_name,col_index,col_len,col_seperator,check_type,check_value,ht_flag,spec_flag from C_FILE_HEADEND_CHECK where filetype_id = :1 order by ht_flag,col_no ";		
		stmt.setSQLString(sql);
		stmt << fileType;
		if(!stmt.execute())
		{
		...
		RecordHTFmt fmt;
		while(stmt>>fmt.number>>fmt.name>>fmt.index>>fmt.len>>fmt.seperator>>fmt.check_type>>fmt.check_value>>fmt.ht_flag>>fmt.spec_flag)
		{
			if(fmt.ht_flag == 'H')
			{
				vfmta.push_back(fmt);
	...
	{
		//cout<<"getRecordHTFilter 数据库查询异常"<<e.what()<<endl;
		memset(erro_msg,0,sizeof(erro_msg));
...
void FormatPlugin::message(MessageParser&  pMessage)
{
	...
	switch(pMessage.getMessageType())
	{
		...
		case MESSAGE_END_FILE:
			  ...
			  if(Bill_fnTime.UpdateDB(szLogStr,conn2))
			  {
			  ...
			  if(Bill_Route.UpdateDB(szLogStr,conn2))
			  {
...
//		}		
		...
		//2013-07-17 强制备份
		//sql = "select var_value from c_process_env where varname = 'INFILE_BAK_FLAG' and source_group=:1 and service=:2 ";
		//stmt.setSQLString(sql);
		//stmt<<sourceGroup<<service;
		//stmt.execute();
		//if(!(stmt>>bak_flag))
		...
		//cout<<"数据源组："<<sourceGroup<<"    service:"<<service<<" 输入相对路径："<<input_path<<"  输出相对路径"<<output_path<<" 错误文件路径: "<<erro_path<<" 超时文件路径："<<timeout_path<<" 格式化未定义话单文件路径："<<other_path<<" 备份标志:"<<bak_flag<<"  备份路径:"<<bak_path<<" SQL存放路径文件:"<<sqlFile<<endl;	
		}catch(SQLException e)
		{
...
/*	 基类已经实现该功能
	 ...
	 //2013-07-19 从核心参数里面读取日志的路径，级别，
	 char szLogPath[PATH_NAME_LEN+1],szLogLevel[10],sql_path[1024];
	 //CString sKeyVal;
	 ...
	 if(param_cfg.bGetMem(sParamName, sKeyVal))
	 {
		...
		sprintf(sqlFile,"%sformat.%d.sql",sql_path,getFlowID());		//fomat_13.sql

	 }
	 else
	 {	
		cout<<"请配置sql文件所在的路径"<<endl;
		return false ;
	 }
...
*/
	...
	{
		  memset(erro_msg,0,sizeof(erro_msg));
		  sprintf(erro_msg,"格式化插件的初始化init() sql err : %s ",e.what());
	...
	DIR *dirptr = NULL; 
	char input_dir[1024],output_dir[1024],erro_dir[1024],timeout_dir[1024],other_dir[1024],bak_dir[1024];
	int rett = -1;
	...
	{		   		
			...
			{
					theJSLog<<"数据源【"<<iter->first<<"】的超时文件路径: "<<timeout_dir<<"不存在，自行创建"<<endi;
			...
			{
					theJSLog<<"数据源【"<<iter->first<<"】的未定义格式话单文件路径: "<<other_dir<<"不存在，自行创建"<<endi;
...
{
	string sql = "select count(*) from D_SCH_FORMAT where filename = :1 ";
	int count = 0;
	Statement stmt = conn.createStatement();
	stmt.setSQLString(sql);
	stmt<<file;
		...
		switch(fmt.check_type)
		{
			...
			case 'D' :          //数字校验 
				  ...
				  for(int j = 0;j<strlen(tmp);j++)
				  {
						...
						{
							memset(erro_msg,0,sizeof(erro_msg));
							sprintf(erro_msg,"文件名校验:数字校验失败,[%s != %s]",tmp,fmt.check_value);
							return CHECK_FAIL;
						}
				  }
				  break;
...
int FormatPlugin::checkRecorHT(char* fileType,char* head,char* tail)
{
	...
	//if(head[0] == '\0' && tail[0] == '\0')   //头尾均为空，不作校验  
	...
	{	
		...
		{
			splitString(head,vfmta[0].seperator,coloum_value_head,false);
		...
		for(int i = 0;i<vfmta.size();i++)
		{
			...
			{	
				...
				strcpy(err_code,"F105");
				sprintf(m_AuditMsg,"file head err F105");
	...
	{	
		...
		{
			splitString(tail,vfmtb[0].seperator,coloum_value_tail,false);
		...
		for(int i = 0;i<vfmtb.size();i++)
		{
			...
			if( checkRule(fmt,tmp) == CHECK_FAIL) 
			{
				...
				strcat(erro_tmp,erro_msg);			
				strcpy(erro_msg,erro_tmp);			
				strcpy(err_code,"F106");
				sprintf(m_AuditMsg,"file tail err F106");
				return CHECK_FAIL;
			}
		}
...
{	
	...
	char szbuff[256],szbuff2[256];
		...
		switch(fmt.check_type)
		{
			...
			case 'H'  :			 //字符串函数截取-变量校验只支持substring函数 内置变量暂时支持 文件名 数据源，文件类型
				  ...
				  {
						theJSLog<<"字符串校验函数表达式配置不合法，此时跳过校验"<<fmt.check_value<<endi;
				  }
...
int FormatPlugin::updateDB(char* sql)
{
	try
	{
		Statement stmt = conn.createStatement();
...
void FormatPlugin::run()
{
	...
	if(drStatus == 1)  //备系统接收到内容后才进行处理
	{
			...
			else if(iStatus == 2)		//跟随模式,默认300s
			{
				...
				if(!flag)
				{
					theJSLog<<"无法查询到文件:"<<fileName<<endi;
					sprintf(m_AuditMsg,"%s not find ",data[1]);

					//需要写调度表吗,只有先写到这儿,需要在这儿仲裁吗
			...
			if(ret == 0)
			{
				theJSLog<<"文件"<<fileName<<"处理成功"<<endi;
			}
			else if(ret == -1)
			{
				theJSLog<<"文件"<<m_szFileName<<"处理失败"<<endi;
			}
			else
			{
				theJSLog<<"系统出错"<<endi;
	...
	{		
			...
			{	
				...
				{	 
					...
					if(rett == 100)
					{		
						//cout<<dir<<"此时文件目录下面没有文件，扫描下个数据源"<<endl;
					...
					memset(m_SerialString,0,sizeof(m_SerialString));
					sprintf(m_SerialString,"%s;%s;%s",it->first,m_szFileName,sqlFile);
					ret = drVarGetSet(m_SerialString);
					if(ret)
					{
						theJSLog<<"同步失败...."<<endi;
...
{			
		...
		char dir[1024],erro_dir[1024],timeout_dir[1024],other_dir[1024],sz_errmsg[1024],szFiletypeIn[10];
		...
		{			
					...
					if(checkFileRepeat(m_szFileName))
					{
							...
							sprintf(erro_msg,"文件重复，调度表中已经存在,错误码：F104");					
							theJSLog.writeLog(0,erro_msg);		//错误码有待更正	
							...
							sprintf(m_AuditMsg,"file repeate F104");
							return -1;
					}
					...
					//cout<<"file_time = "<<file_time<<" "<<it->second.file_begin<<"  "<<it->second.file_length<<endl;
					
					//扫描到文件后将其注册
					memset(sql,0,sizeof(sql));
					sprintf(sql,"insert into D_SCH_FORMAT(SOURCE_ID,FILENAME,FILESIZE,RECIEVE_TIME,DEAL_FLAG,RECORD_COUNT,FILE_ID,FILE_TIME) values('%s','%s',%d,'%s','%c',%d,%ld,'%s')",m_szSourceID,m_szFileName,fileInfo.st_size,currTime,state,record_num,file_id,file_time);					
					writeSQL(sql);
					...
					if(checkFileName(m_szSourceID,m_szFileName) == CHECK_FAIL)
					{
							...
							sprintf(m_AuditMsg,"file name err F101");
							return -1;
				    ...
				    {		
						...
						switch(res->getAnaType())   //根据格式后的类型判断记录的处理方式
						{
							...
							case eFmtErr:			//格式化错单
								 memset(erro_msg,0,sizeof(erro_msg));
								 sprintf(erro_msg,"格式化错单");
								 theJSLog.writeLog(0,erro_msg);	
							
								 sprintf(m_AuditMsg,"format err rcd ");
							...
							case eFmtTimeOut:		//格式化超时单，入timeout文件
								 memset(erro_msg,0,sizeof(erro_msg));
								 sprintf(erro_msg,"格式化超时单");
								 theJSLog.writeLog(0,erro_msg);

								 sprintf(m_AuditMsg,"format timeout rcd ");
							...
							case eFmtOther:			//格式化未定义格式话单，入otherformat文件
								 memset(erro_msg,0,sizeof(erro_msg));
								 sprintf(erro_msg,"未定义的格式化单");
								 theJSLog.writeLog(0,erro_msg);
								
								 sprintf(m_AuditMsg,"format other rcd ");
					...
					if(deal_flag == 1)								//格式化文件出了错，记录不要
					{
						...
						memset(sql,0,sizeof(sql));
						state = 'E';
						sprintf(sql,"update D_SCH_FORMAT set DEAL_FLAG='%c',DEAL_TIME='%s',RECORD_COUNT=%d where SOURCE_ID='%s' and FILENAME='%s' ",state,currTime,record_num,m_szSourceID,m_szFileName);
						writeSQL(sql);
			...
			/*	
					...
					else 
					{
						...
						rename(strcat(tmp,".tmp"),outFileName);
					}
	  ...
	  */		
		...
		}catch(jsexcp::CException e)			//抛异常时文件怎么处理,暂时放在源目录下面
		{			
					rollBackSQL();
					memset(erro_msg,0,sizeof(erro_msg));
...
int FormatPlugin::dealAuditResult()
{
		int ret = 0;
		char state,tmp[1024];
		
		cout<<"原始文件:"<<fileName<<endl;
		...
		{		
				...
				memset(tmp,0,sizeof(tmp));
				strcpy(tmp,outFileName);
				if(rename(strcat(tmp,".tmp"),outFileName))
				{
					memset(erro_msg,0,sizeof(erro_msg));
