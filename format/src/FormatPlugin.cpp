...
int FormatPlugin::getmapAdjCycleBusi(string source)
{
	string sql;
	try
	{	
		Statement stmt = conn.createStatement();
	    sql = "select MONTH_ADJ from C_CYCLE_ADJ_DEFINE where source_id = :1 and vail_flag='Y' ";		
		stmt.setSQLString(sql);
		stmt << source;
		if(!stmt.execute())
		{
...
void FormatPlugin::message(MessageParser&  pMessage)
{
	...
	switch(pMessage.getMessageType())
	{
		...
		//加MsgDirection
			...
			sprintf(Field[MAX_FIELD_COUNT-19],"%s",rate_cycle_org);	//2014-07-02
...
/*********获取格式化插件初始化所需要的数据源组,service*** 2013-03-11 add by hed*********************************************/
	...
	try{
		...
		{
			theJSLog<<"格式化错误文件路径变量[FMT_ERR_DIR]没有在c_process_env中配置"<<endw;
			return false; 
		}
		...
		{
			theJSLog<<"格式化超时文件路径变量[FMT_TIMEOUT_DIR]没有在c_process_env中配置"<<endw;
			return false ;
		}
		...
		{
			theJSLog<<"格式化未定义格式话单文件路径[FMT_OTHER_DIR]没有在c_process_env中配置"<<endw;
			return false ;
		}
		...
		{
			theJSLog<<"请在表c_process_env配置格式化模块的备份目录:FMT_BACKUP_DIR"<<endw;
			return false;
		}
		...
		{
			theJSLog<<"init 数据库查询异常:"<<e.what()<<endw;
			return false ;
		}
	...
	{		   		
			...
			{		 
					...
					{
						memset(erro_msg,0,sizeof(erro_msg));
						sprintf(erro_msg,"数据源[%s]的输出话单文件文件路径[%s]不存在，自行创建失败",iter->first,output_dir);
						theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错
						return false;
					}
		...
		switch(fmt.check_type)
		{
			case 'C' :            //常量校验
				  ...
				  if(strcmp(fmt.check_value,tmp) != 0)
				  {
					  ...
					  sprintf(mdrDeal.m_AuditMsg,"%s:%c:%s",tmp,fmt.check_type,fmt.err_code);

					  memset(erro_msg,0,sizeof(erro_msg));
					  sprintf(erro_msg,"[%s]文件名校验:常量校验失败,[%s != %s]",m_szFileName,tmp,fmt.check_value);
					  return CHECK_FAIL;
				  }
			...
			case 'Z' :			 //日期格式校验支持6位 8位 10位 12位 14位
				  ...
				  if(len != strlen(fmt.check_value))  //要校验的日期字符串截取的长度和设置的日期格式不一样，肯定校验失败
				  {
						...
						memset(erro_msg,0,sizeof(erro_msg));
					    sprintf(erro_msg,"[%s]文件名校验:要校验的日期字符串截取的长度和设置的日期格式不一致, [%s != %s]",m_szFileName,tmp,fmt.check_value);
						return CHECK_FAIL;
				  }
				  ...
				  for(int j = 0;j<len;j++)
				  {
						if(tmp[j] > '9' || tmp[j] < '0')
						{
							...
							memset(erro_msg,0,sizeof(erro_msg));
							sprintf(erro_msg,"[%s]文件名校验： 日期校验之日期校验失败, 日期必须全为数字 [%s] ",m_szFileName,tmp);
							return CHECK_FAIL;
						}
				  }
				  ...
				  if(len == 6||len == 8||len ==10||len ==12||len ==14)
				  {
						if(checkDate(tmp) == CHECK_FAIL)
						{
								...
								memset(erro_msg,0,sizeof(erro_msg));
								sprintf(erro_msg,"[%s]文件名校验:日期时间不合法,[%s]",m_szFileName,tmp);
								return CHECK_FAIL;
						}
				  }
				  else 
				  {
					  ...
					  memset(erro_msg,0,sizeof(erro_msg));
					  sprintf(erro_msg,"[%s]文件名校验:日期格式校验失败,长度必须为6 8 10  12 14,[%s]",m_szFileName,tmp);
					  return CHECK_FAIL;
				  }
			...
			case 'D' :          //数字校验 
				  ...
				  for(int j = 0;j<strlen(tmp);j++)
				  {
						if(tmp[j] > '9' || tmp[j] < '0')
						{
							...
							memset(erro_msg,0,sizeof(erro_msg));
							sprintf(erro_msg,"[%s]文件名校验:数字校验失败,[%s]",m_szFileName,tmp);
							return CHECK_FAIL;
						}
				  }
				  break;
...
{
	theJSLog<<"话单文件类型："<<fileType<<"  头记录："<<head<<"  尾记录："<<tail<<endi;
	...
	{	
		...
		{
			file_head.clear();
			splitString(tmp,vfmta[0].seperator,file_head,false,false);  //不跳过空串
			flag = 1;
		...
		for(int i = 0;i<vfmta.size();i++)
		{
			...
			{	
				sprintf(erro_tmp,"[%s]文件记录头校验:",m_szFileName);
				strcat(erro_tmp,erro_msg);
				strcpy(erro_msg,erro_tmp);

				strcpy(err_code,fmt.err_code);		//字段I的值在做某种类型的校验失败
				sprintf(mdrDeal.m_AuditMsg,"%s:%c:%s:%s",fmt.name,fmt.check_type,tmp,fmt.err_code);
	...
	{	
		...
		for(int i = 0;i<vfmtb.size();i++)
		{
			...
			if( checkRule(fmt,tmp) == CHECK_FAIL) 
			{
				sprintf(erro_tmp,"[%s]文件记录尾校验:",m_szFileName);
				strcat(erro_tmp,erro_msg);			
				strcpy(erro_msg,erro_tmp);	
...
{	
	...
	switch(fmt.check_type)
	{
		...
		case 'D' :          //数字校验,改为数值校验
			  ...
			  for(int j = 0;j<len;j++)
			  {
					if((j == 0) && column_value[j] == '-')		//负数情况
					{
						continue;
					}
					
					//if((j != 0 || j != (len-1)) && column_value[j] == '.')	//小数情况 .36, 45. 这种也合法
					//{
					//	continue;
					//}
					...
					if(column_value[j] > '9' || column_value[j] < '0')
					{
						//strcpy(err_code,fmt.err_code);
						memset(erro_msg,0,sizeof(erro_msg));
						sprintf(erro_msg,"数字校验失败,必须为数值,[%s]",column_value);
						return CHECK_FAIL;
					}
			  }
			  break;
...
void FormatPlugin::run()
{
	...
	char tmp[JS_MAX_FILEFULLPATH_LEN],filter[50],inputFilePath[JS_MAX_FILEFULLPATH_LEN];
...
{	
	...
	if(mdrDeal.mdrParam.drStatus == 1)  //备系统接收到内容后才进行处理
	{
		...
		if(!(dbConnect(conn)))
		{
			...
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"run() 连接数据库失败,dr_AbortIDX() ");
			theJSLog.writeLog(LOG_CODE_DB_CONNECT_ERR,erro_msg);		//连接数据库失败	
			continue ;
		}
		
		memset(sql,0,sizeof(sql));
		strcpy(sql,"select S_FILE_RECEIVED.NEXTVAL from dual");		//从数据库得FileID,保证两边都用的一致,便于两边切换
		...
		//获取同步变量
		vector<string> data;		
		splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串数据源ID,文件名,sqlFile
		...
		petri_status = atoi(data[3].c_str());		//备系统的状态根据主系统来定	
		...
		{	
			...
			if(!flag)
			{
				...
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"无法查询到文件:%s,dr_AbortIDX() ",fileName);
	...
	{			
		...
		//{	
			...
			{	
				...
				{
					theJSLog<<"获取文件信息失败:"<<strerror(errno)<<endw;
					continue;			//表示获取文件信息失败
				}
				...
				memset(mdrDeal.m_SerialString,0,sizeof(mdrDeal.m_SerialString));
				sprintf(mdrDeal.m_SerialString,"%s;%s;%ld;%d",it->first,m_szFileName,file_id,petri_status);
				ret = mdrDeal.drVarGetSet(mdrDeal.m_SerialString);
				if(ret)
				{
					theJSLog<<"同步失败...."<<endw;
...
{			
		...
		char dir[JS_MAX_FILEFULLPATH_LEN],tmp[JS_MAX_FILEFULLPATH_LEN],outFilePath[JS_MAX_FILEFULLPATH_LEN],szFiletypeIn[10];
		char szBuff[JS_MAX_RECORD_LEN],head[JS_MAX_RECORD_LEN],tail[JS_MAX_RECORD_LEN];	
		char state = 'W';
		//ofstream ff ;
...
/*
		...
		{
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"文件[%s]改名失败: %s",fileName,strerror(errno));
...
*/
		...
		{	
			...
			if(checkFileRepeat(m_szFileName))
			{
				...
				sprintf(erro_msg,"文件[%s]重复,调度表中已经存在,F104",m_szFileName);					
				theJSLog.writeLog(LOG_CODE_FILE_NAME_REPEAT ,erro_msg);			
				
				memset(sql,0,sizeof(sql));		
				getCurTime(currTimeB);    //获取当前文件时间
				state = 'E';
				...
				sprintf(sql,"insert into %s (SOURCE_ID,FILENAME,FILESIZE,RECIEVE_TIME,DEAL_TIME,DEAL_FLAG,RECORD_COUNT,FILE_ID,FILE_TIME,SERV_CAT_ID) values('%s','%s',%ld,'%s','%s','%c',%d,%ld,'%s','%s')",mConfParam.szSchCtlTabname,m_szSourceID,m_szFileName,0,currTimeA,currTimeB,state,0,file_id,file_time,mServCatId);
				//writeSQL(sql);
				...
				memset(sql,0,sizeof(sql));
				//getCurTime(currTimeB);    //获取当前文件时间
				sprintf(sql,"insert into D_ERRFILE_INFO(FILENAME,SOURCE_ID,DEAL_TIME,ERR_MSG,ERR_CODE,ERR_COL,ERR_LINE,ERR_SEQ,STATE) values('%s','%s','%s','%c','%s',%d,%d,%ld,'%c')",m_szFileName,m_szSourceID ,currTimeB,'F',err_code,0,0,file_id,state);		
				//writeSQL(sql);
				m_vsql.push_back(sql);
							
				sprintf(mdrDeal.m_AuditMsg,"%s:%s",m_szFileName,"F104");
				return -1;
			}	
			...
			//writeSQL(sql);
			//m_vsql.push_back(sql);			
			//sprintf(sql,"insert into %s (SOURCE_ID,FILENAME,FILESIZE,RECIEVE_TIME,DEAL_TIME,DEAL_FLAG,RECORD_COUNT,FILE_ID,FILE_TIME,SERV_CAT_ID) values('%s','%s',%ld,'%s','%s','%c',%d,%ld,'%s','%s')",mConfParam.szSchCtlTabname,m_szSourceID,m_szFileName,fileInfo.st_size,currTimeA,currTimeB,state,record_num,file_id,file_time,mServCatId);
			...
			//2013-10-24 修改获取账期方式 D_CHECK_FILE_DETAIL
			char rate_cycle_tmp[6+1],fz_flag ;
			memset(rate_cycle_tmp,0,sizeof(rate_cycle_tmp));
			memset(sql,0,sizeof(sql));
			sprintf(sql,"select rate_cycle,cycle_flag from D_CHECK_FILE_DETAIL where source_id = '%s' and check_type = 'AUD' and file_time = '%s' and content = '%s'",m_szSourceID,file_time,m_szFileName);
			//stmt = conn.createStatement();
			stmt.setSQLString(sql);
			stmt.execute();
			stmt>>rate_cycle_tmp>>fz_flag;
			if(rate_cycle_tmp[0] == '\0')
			{
				theJSLog<<"表D_CHECK_FILE_DETAIL没有该文件记录,暂时从文件名上面获取账期"<<endi;
				fz_flag = '0' ;		//默认不封帐
				strcpy(rate_cycle_org,rate_cycle);
			}
			else
			{
				memset(rate_cycle,0,sizeof(rate_cycle));
				strcpy(rate_cycle,rate_cycle_tmp);
				strcpy(rate_cycle_org,rate_cycle_tmp);
			}
			...
			stmt.execute();
			stmt>>cycle_flag;
			if(cycle_flag == 'Y')			//封帐文件或者已封帐
			{
				if(fz_flag == '0')		//2014-01-10当前封帐文件不应该当成错误文件
				{
					//map< string,C_ADJ_CYCEL_DEF >::const_iterator iter = mapAdjCycleBusi.find(source);
					//2014-07-01 判断当前业务是否是可以调整的账期情况
					if(mapAdjCycleBusi.find(m_szSourceID) == mapAdjCycleBusi.end())
					{		
					
						strcpy(err_code,"F120");
						sprintf(erro_msg,"[%s],当前账期[%s]已封帐,F120",m_szFileName,rate_cycle);					
						theJSLog.writeLog(LOG_CODE_FILE_NAME_REPEAT ,erro_msg);	
						...
						sprintf(mdrDeal.m_AuditMsg,"%s:%s",m_szFileName,"F120");
						return -1;
					}
					else		//此时实际计算的账期需修改 保留两个账期,一个原始账期,一个修改要调整的那个账期(入库的账期) 可能存在的问题:格式化和核对之间可能间隔比较大 核对封帐,但格式化还没处理该账期文件
					{
						strcpy(rate_cycle_org,rate_cycle);
						...
						strcpy(rate_cycle,rate_cycle_tmp);
						theJSLog<<"file["<<m_szFileName<<"] adj to ratecyle="<<rate_cycle_tmp<<endi;
					}
				}
				else
				{
					theJSLog<<"当前文件为封帐文件..."<<endi;
				}
			}
			...
			{					
				...
				sprintf(erro_msg,"文件[%s]打开错误:%s,F103",m_szFileName,strerror(errno));					
				theJSLog.writeLog(LOG_CODE_FILE_NAME_REPEAT ,erro_msg);			
				...
				sprintf(mdrDeal.m_AuditMsg,"%s:%s",m_szFileName,"F103");
				return -1;
			}
			in.close();
			...
			if(record_num == 0)
			{
				theJSLog<<"["<<m_szFileName<<"]该文件空文件..."<<endi;
				sprintf(mdrDeal.m_AuditMsg,"%s;%s,%d",m_szSourceID,m_szFileName,record_num);
			...
			{		
				...
				switch(res->getAnaType())   //根据格式后的类型判断记录的处理方式
				{
					...
					case eFmtErr:			//格式化错单
						 memset(erro_msg,0,sizeof(erro_msg));
						 sprintf(erro_msg,"[%s]格式化错单:%d[%s|%s]",m_szFileName,record_num,res->getRuleType(),res->getReason());
						 theJSLog.writeLog(LOG_CODE_FORMAT_ERR_RCD ,erro_msg);	
					
						 sprintf(mdrDeal.m_AuditMsg,"format err rcd:%d[%s|%s]",record_num,res->getRuleType(),res->getReason());
					...
					case eFmtTimeOut:		//格式化超时单，入timeout文件
						 memset(erro_msg,0,sizeof(erro_msg));
						 sprintf(erro_msg,"[%s]格式化超时单:%d[%s|%s]",m_szFileName,record_num,res->getRuleType(),res->getReason());
						 theJSLog.writeLog(LOG_CODE_FORMAT_TIMEOUT_RCD,erro_msg);

						 sprintf(mdrDeal.m_AuditMsg,"format timeout rcd:%d[%s|%s]",record_num,res->getRuleType(),res->getReason());
					...
					case eFmtOther:			//格式化未定义格式话单，入otherformat文件
						 memset(erro_msg,0,sizeof(erro_msg));
						 sprintf(erro_msg,"[%s]未定义的格式化单:%d[%s|%s]",m_szFileName,record_num,res->getRuleType(),res->getReason());
						 theJSLog.writeLog(LOG_CODE_FORMAT_OTHER_RCD,erro_msg);
						
						 sprintf(mdrDeal.m_AuditMsg,"format other rcd:%d[%s|%s]",record_num,res->getRuleType(),res->getReason());
					...
					default :
						...
						sprintf(mdrDeal.m_AuditMsg,"format undefine rcd ");
						break;
				}
			...
			if(deal_flag == 1)								//格式化文件出了错，记录不要
			{
				...
				sprintf(sql,"insert into %s (SOURCE_ID,FILENAME,FILESIZE,RECIEVE_TIME,DEAL_TIME,DEAL_FLAG,RECORD_COUNT,FILE_ID,FILE_TIME,SERV_CAT_ID) values('%s','%s',%ld,'%s','%s','%c',%d,%ld,'%s','%s')",mConfParam.szSchCtlTabname,m_szSourceID,m_szFileName,0,currTimeA,currTimeB,state,record_num,file_id,file_time,mServCatId);
				//writeSQL(sql);
				m_vsql.push_back(sql);
				...
				if(res->getAnaType() == eFmtErr)		
				{
					memset(sql,0,sizeof(sql));
					state = 'W';
					sprintf(sql,"insert into D_ERRFILE_INFO(FILENAME,SOURCE_ID,DEAL_TIME,ERR_MSG,ERR_CODE,ERR_COL,ERR_LINE,ERR_SEQ,STATE) values('%s','%s','%s','%c','%s',%d,%d,%ld,'%c')",m_szFileName,m_szSourceID ,currTimeB,'F',"F110",0,iTotalNum,file_id,'W');
					//writeSQL(sql);
					m_vsql.push_back(sql);
				}
			...
			if(ret)
			{
				theJSLog<<"写输出文件失败["<<outFileName<<"]"<<endw;
				state = 'E';
			}
			record_array.clear();     								
			...
			strcat(tmp,".tmp");
			stat(tmp,&fileInfo);				//获取输出文件大小 	
			
			sprintf(mdrDeal.m_AuditMsg,"%s;%s,%d,%d",m_szSourceID,m_szFileName,record_num,fileInfo.st_size);
			
			//2015-02-03 登记改为输入文件大小
		...
		{			
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"[%s]dealFile() 格式化出错 %s",m_szFileName,e.GetErrMessage());
			theJSLog.writeLog(LOG_CODE_FIELD_CONVERT_ERR,erro_msg);		//字段转换出错
					
			sprintf(mdrDeal.m_AuditMsg,"%s;%s",mdrDeal.m_AuditMsg,e.GetErrMessage());
...
/*
		...
		{ 
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"[%s]dealFile() 数据库操作异常:%s (%s)",m_szFileName,e.what(),sql);
...
int FormatPlugin::writeFile(char* filename)
{
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"writeFile 文件[%s]打开出错:%s ",filename,strerror(errno));
...
int FormatPlugin::dealAuditResult(int result)
{
	...
	{	
		...
		if(ret != 3)							//2013-11-07仲裁超时不移动文件
		{
			char erro_dir[JS_MAX_FILEFULLPATH_LEN];						
			//memset(tmp,0,sizeof(tmp));
			//strcpy(tmp,fileName);
			//strcat(tmp,".tmp");
			memset(erro_dir,0,sizeof(erro_dir));
			...
			strcat(erro_dir,mConfParam.szErroPath);
			theJSLog<<"将文件移到仲裁失败目录:"<<erro_dir<<endi;
			if(rename(fileName,strcat(erro_dir,m_szFileName)))
			{
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"移动源文件到错误目录[%s]失败: %s",erro_dir,strerror(errno));
				theJSLog.writeLog(LOG_CODE_FILE_MOVE_ERR,erro_msg);
			}
		}

		memset(tmp,0,sizeof(tmp));					//删除可能存在的输出临时文件
		strcpy(tmp,outFileName);
		strcat(tmp,".tmp");
	...
	else							//仲裁成功,提交sql 临时改正式,文件移到失败还是备份看处理结果
	{		
		if(result)					//抛异常2013-11-01,删除临时文件,也有可能有输出的临时文件,没有考虑文件改名失败情况
		{
			char erro_dir[JS_MAX_FILEFULLPATH_LEN];
			//memset(tmp,0,sizeof(tmp));
			//strcpy(tmp,fileName);
			...
			strcpy(erro_dir,it->second.szSourcePath);			 
			strcat(erro_dir,mConfParam.szErroPath);
			theJSLog<<"将文件移到处理失败目录:"<<erro_dir<<endi;
			if(rename(fileName,strcat(erro_dir,m_szFileName)))
			{
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"移动源文件到错误目录[%s]失败: %s",erro_dir,strerror(errno));
			...
			{
				theJSLog<<"数据库状态为只读态,暂时写sql文件"<<endi;
				string ssql;
...
{	
	...
	{ 
		...
		memset(erro_msg,0,sizeof(erro_msg));
		sprintf(erro_msg,"[%s] updateDB 数据库出错:%s (%s)",m_szFileName,e.what(),ssql);
		theJSLog.writeLog(LOG_CODE_DB_EXECUTE_ERR,erro_msg);		//发生sql执行异常
