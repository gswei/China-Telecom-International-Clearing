...
*/
		...
		{	
					...
					if(ret)
					{
						theJSLog<<"写文件失败["<<outFileName<<"]"<<endw;
						state = 'E';
					}
					record_array.clear();     								
...
int FormatPlugin::dealAuditResult(int result)
{
		...
		if(ret)										//回滚数据库,文件移到错误目录,生成的文件删除
		{
			...
			if(ret != 3)							//2013-11-07仲裁超时不移动文件
			{
				...
				theJSLog<<"将文件移到失败目录:"<<erro_dir<<endi;
...
{	
	...
	{ 
		...
		memset(erro_msg,0,sizeof(erro_msg));
		sprintf(erro_msg,"[%s] updateDB 数据库出错:%s (%s),暂时写sql文件",m_szFileName,e.what(),ssql);
		theJSLog.writeLog(LOG_CODE_DB_EXECUTE_ERR,erro_msg);		//发生sql执行异常
