...
int day_or_month = 1;		//日核对还是月核对
bool mFullPathCdr = false;  //2014-01-11话单的接收路径是全路径还是相对路径
...
//#######################################################################
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置源文件接收路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置源文件接收路径[file.check.receive_path]"<<endw;
			return false ;
		}	 
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置源文件的输出路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置源文件的输出路径[file.check.output_path]"<<endw;
			return false ;
		}	
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置核对文件的输入路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置核对文件的输入路径[file.check.input_path]"<<endw;
			return false ;
		}	 
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置月核对文件的输入路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置月核对文件的输入路径[file.check.month_input_path]"<<endw;
			return false ;
		}	 
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置核对模块的备份路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置核对模块的备份路径[file.check.bak_path]"<<endw;
			return false ;
		}	 	
	
		sprintf(sParamName, "file.check.fail_path");			//核对文件失败目录
		if(param_cfg.bGetMem(sParamName, sKeyVal))
		{
		...
		{	
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"请在核心参数里配置核对文件的核对失败存放路径,参数名:%s",sParamName);
			theJSLog.writeLog(LOG_CODE_ENV_MISSING,erro_msg);

			//theJSLog<<"请在核心参数里配置核对文件的核对失败存放路径[file.check.fail_path]"<<endw;
			return false ;
		}	 
		...
		if(mFullPathCdr)
		{
			...
			{
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"原始话单文件接收目录[%s]打开失败",receive_path);
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"核对文件输入目录[%s]打开失败",input_path);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

	 		//theJSLog<<"核对文件输入目录["<<input_path<<"]打开失败"<<endw;
	 		return false ;
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"月汇总文件输入目录[%s]打开失败",month_input_path);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

	 		//theJSLog<<"月汇总文件输入目录["<<month_input_path<<"]打开失败"<<endw;
	 		return false ;
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"核对文件备份目录[%s]打开失败",bak_path);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

	 		//theJSLog<<"备份目录["<<bak_path<<"]打开失败"<<endw;
	 		return false ;
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"核对失败目录[%s]打开失败",fail_path);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

	 		//theJSLog<<"核对失败目录["<<fail_path<<"]打开失败"<<endw;
	 		return false ;
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"日核对备份目录[%s]打开失败",bak_path1);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

			//theJSLog<<"备份目录["<<bak_path1<<"]打开失败"<<endw;
			return false;
		}
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"月核对备份目录[%s]打开失败",bak_path2);
			theJSLog.writeLog(LOG_CODE_DIR_OPEN_ERR,erro_msg); //打开目录出错

			//theJSLog<<"备份目录["<<bak_path2<<"]打开失败"<<endw;
			return false ;
		}
	...
	while(1)
	{
		...
		if(mdrDeal.mdrParam.drStatus == 1 )  //备系统
		{
		   ...
		   if(db_status != petri_status_tmp)
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串文件名,数据库状态,文件类型
...
/*			if(db_status != petri_status_tmp)
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串文件名,数据库状态
...
*/
	...
	else
	{
		...
		if(cnt == 10 ||ret == -1)		//文件超时或者文件格式错误等
		{
			...
			{
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"本次核对文件[%s]核对失败,失败个数:(%d)",m_szFileName,ret);
		...
		memset(mdrDeal.m_SerialString,0,sizeof(mdrDeal.m_SerialString));		//文件全部找齐了
...
/*			if(db_status != petri_status_tmp)
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串:文件名,数据库状态
...
*/
	...
	if(ret)
	{
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"文件[%s]月核对失败",m_szFileName);
			theJSLog.writeLog(LOG_CODE_FILE_ERR_CHECK,erro_msg);
		}
	...
	if(mdrDeal.mdrParam.drStatus != 1)				//主系统发送同步串
	{
		//发送同步信息
		memset(mdrDeal.m_SerialString,0,sizeof(mdrDeal.m_SerialString));
	...
	{	
		...
		{	
			...
			if(iter == monthSumMap.end())
			{
				in.close();
				memset(erro_msg,0,sizeof(erro_msg));
				sprintf(erro_msg,"checkMonthFile 配置表中没有找到月汇总文件文件类型为[%s]的信息",rc.file_type);
				theJSLog.writeLog(LOG_CODE_FILE_ERR_RCD,erro_msg);
				return -1;
			}
...
/* 2014-10-13 月核对文件应该在封账之前发过来,不需要条件
				...
				{
					memset(erro_msg,0,sizeof(erro_msg));
					sprintf(erro_msg,"checkMonthFile 数据源[%s]当月账期[%s]还没封帐...",iter->second.source_id,month);
...
*/
				...
				//查询表，求出对应表中的数据与核对记录进行合计 费用可能有多项
				memset(sql,0,sizeof(sql));

				//2014-08-11长途业务跨账期表特殊处理
				if(strcmp("TOLL",ff.file_type.c_str()) == 0)
				{
					...
					if(strncmp(next_month,currTime,6) == 0)			//就是当前账期
					{
				...
				if(cdr_cnt != fileList2[i].cdr_count)
				{
					...
					memset(erro_msg,0,sizeof(erro_msg));
					sprintf(erro_msg,"文件类型：[%s] 通话次数核对失败 %ld != %ld",ff.file_type,cdr_cnt,fileList2[i].cdr_count);
					theJSLog.writeLog(LOG_CODE_FILE_ERR_CHECK,erro_msg);
				...
				if(cdr_duration != fileList2[i].cdr_duration)
				{
					...
					memset(erro_msg,0,sizeof(erro_msg));
					sprintf(erro_msg,"文件类型：[%s] 通话时长核对失败 %ld != %ld",ff.file_type,cdr_duration,fileList2[i].cdr_duration);
					theJSLog.writeLog(LOG_CODE_FILE_ERR_CHECK,erro_msg);
				...
				if(totalFee != fileList2[i].cdr_fee)
				{
					...
					memset(erro_msg,0,sizeof(erro_msg));
					sprintf(erro_msg,"文件类型：[%s] 费用核对失败 %ld != %ld",ff.file_type,totalFee,fileList2[i].cdr_fee);
					theJSLog.writeLog(LOG_CODE_FILE_ERR_CHECK,erro_msg);
