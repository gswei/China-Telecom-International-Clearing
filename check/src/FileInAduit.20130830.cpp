...
int FileInAduit::check_before_file()
{
	...
	try
	{
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"select count(1) from d_check_file_detail where check_type  = 'AUD' and check_flag = 'N'");
		stmt.setSQLString(sql);
		stmt.execute();
		stmt>>cnt;
		if(cnt == 0)
		...
		string f1,f2;
		memset(sql,0,sizeof(sql));
		sprintf(sql,"select check_file,content from d_check_file_detail where check_type  = 'AUD' and check_flag = 'N' and rownum < 51");
		...
		for(map< string,vector<string> >::iterator	it = failFileMap.begin();it != failFileMap.end();++it)
		{
			...
			for(int i = 0;i<vv.size();i++)
			{
				...
				if(ret)									//核对成功就更新原先的记录
				{
					...
					sprintf(m_AuditMsg,"%s%s,Y|",m_AuditMsg,vv[i]);
				}
				else
				{
					sprintf(m_AuditMsg,"%s%s,N|",m_AuditMsg,vv[i]);
				}
			}
	...
	{
		memset(erro_msg,0,sizeof(erro_msg));
		sprintf(erro_msg,"check_before_file() 数据库操作异常%s [%s]",e.what(),sql);
		theJSLog.writeLog(LOG_CODE_DB_EXECUTE_ERR,erro_msg);		//发生sql执行异常
	}	
...
{			
	int ret = 0,flag = 0;
	CF_CFscan scan2;
	char tmp[1024],tmp2[1024], *p,receive_dir[1024];  //out_path[1024],receive_dir[1024],filter[256];
	string source_id;
...
void FileInAduit::run(int flag)
{
	...
	{	//如果是备系统
		...
		if( drStatus == 1 )  //备系统
		{
			...
			isWriteSQLFileByMain(data[1].c_str());	//备系统的sqlFile是通过主系统传过来的
			
			memset(fileName,0,sizeof(fileName));
			if(flag)strcpy(fileName,input_path);
			else strcpy(fileName,month_input_path);		
			...
			else if(iStatus == 2)		//跟随模式,默认300s
			{
				...
				if(!flag)
				{
					...
					//需要写调度表吗,只有先写到这儿
					memset(m_AuditMsg,0,sizeof(m_AuditMsg));
					sprintf(m_AuditMsg,"%s not find",data[0]);
					if(!IsAuditSuccess(m_AuditMsg))
					{
						return;
					}
		...
		while(1)		
		{
				...
				//发送同步串[核对文件名,sqlFile文件名]....................
				memset(m_SerialString,0,sizeof(m_SerialString));
				sprintf(m_SerialString,"%s;%s",m_szFileName,sqlFile);
				ret = drVarGetSet(m_SerialString);
				if(ret)
				{
					theJSLog<<"同步失败...."<<endi;
...
int FileInAduit::dealFile()
{
	...
	{	
		...
		{		
				...
				sprintf(m_AuditMsg,"%s:%s:%c|",m_AuditMsg,fileList[i].fileName,check_flag);
		...
		if(!IsAuditSuccess(m_AuditMsg))			//仲裁失败,回滚数据库,文件需要移到失败目录吗
		{
				rollBackSQL();
		...
		commitSQLFile();
		char out_dir[1024],receive_dir[1024];
