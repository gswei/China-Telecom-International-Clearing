...
{ 
	...
	try
	{
		...
		sql = "select check_type,sum_table,cdr_count,cdr_duration,cdr_fee,rate_cycle from c_check_file_config ";
		stmt = conn.createStatement();
		stmt.setSQLString(sql);
		stmt.execute();
		while(stmt>>fmt.check_type>>fmt.sum_table>>fmt.cdr_count>>fmt.cdr_duration>>fee>>fmt.rate_cycle)
		{
			theJSLog<<"check_type="<<fmt.check_type<<"  sum_table="<<fmt.sum_table<<"   cdr_duration="<<fmt.cdr_duration<<" fee="<<fee<<"  rate_cycle="<<fmt.rate_cycle<<endi;
...
void FileInAduit::run(int flag)
{
	...
	{	
		...
		if(mdrDeal.mdrParam.drStatus == 1 )  //备系统
		{
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串文件名,sqlFile文件名
	...
	else
	{
		...
		sprintf(mdrDeal.m_SerialString,"%s;%d",m_szFileName,petri_status);
		ret = mdrDeal.drVarGetSet(mdrDeal.m_SerialString);
		if(ret)
		{
			theJSLog<<"同步失败...."<<endw;
...
void FileInAduit::run2()
{
	...
	{	
		...
		if( mdrDeal.mdrParam.drStatus == 1 ) 
		{
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrDeal.m_SerialString,";",data,false,false);  //发送的字符串:文件名
...
int FileInAduit::checkMonthFile()
{
	...
	{	
		...
		{		
				...
				Statement stmt = conn.createStatement();
				memset(sql,0,sizeof(sql));
				sprintf(sql,"select count(*) from D_CHECK_FILE_DETAIL where source_id = '%s' and check_type = 'AUD' and rate_cycle = '%s' and cycle_flag = '1'",ff.file_type,month);
				stmt.setSQLString(sql);
				stmt.execute();
				stmt>>ret;
				if(ret == 0)
				...
				string ft;
				memset(sql,0,sizeof(sql));
				sprintf(sql,"select distinct(file_time) from D_CHECK_FILE_DETAIL where source_id = '%s' and check_type = 'AUD' and rate_cycle = '%s'  order by  file_time desc ",ff.file_type,month);
				stmt.setSQLString(sql);
				stmt.execute();
				while(stmt>>ft)
				{
