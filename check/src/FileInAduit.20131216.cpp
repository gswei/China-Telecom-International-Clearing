...
void FileInAduit::run(int flag)
{
	...
	{	
		...
		if( mdrParam.drStatus == 1 )  //备系统
		{
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrParam.m_SerialString,";",data,false,false);  //发送的字符串文件名,sqlFile文件名
	...
	else
	{
		...
		memset(mdrParam.m_SerialString,0,sizeof(mdrParam.m_SerialString));		//文件全部找齐了
		sprintf(mdrParam.m_SerialString,"%s;%d",m_szFileName,petri_status);
		ret = drVarGetSet(mdrParam.m_SerialString);
		if(ret)
		{
			theJSLog<<"同步失败...."<<endw;
		...
		//}
		sprintf(mdrParam.m_AuditMsg,"%d",fileList.size());	//2013-10-20
...
void FileInAduit::run2()
{
	...
	{	
		...
		if( mdrParam.drStatus == 1 ) 
		{
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrParam.m_SerialString,";",data,false,false);  //发送的字符串:文件名
	...
	if(mdrParam.drStatus != 1)				//主系统发送同步串
	{
		//发送同步信息
		memset(mdrParam.m_SerialString,0,sizeof(mdrParam.m_SerialString));
		sprintf(mdrParam.m_SerialString,"%s;%d",m_szFileName,petri_status);
	...
	{
		sprintf(mdrParam.m_AuditMsg,"%s%s,%ld,%ld,%.2f|",mdrParam.m_AuditMsg,fileList2[i].file_type,fileList2[i].cdr_count,fileList2[i].cdr_duration,fileList2[i].cdr_fee);
	}
...
int main(int argc,char** argv)
{
	...
	cout<<"*     last update time : 2013-12-03 by  hed	  "<<endl;
	cout<<"********************************************** "<<endl;
