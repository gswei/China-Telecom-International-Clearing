...
class FileInAduit : public PS_Process
{
   ...
   private:
	 ...
	 char receive_path[1024];	 //原文件接收路径
	 char output_path[1024];     //原文件输出路径

	 char input_path[1024];     ////核对文件输入路径	
	 char month_input_path[1024];	//月汇总文件输入路径

	 char bak_path1[1024];	    //核对文件备份路径
	 char bak_path2[1024];	    //月汇总文件备份路径
