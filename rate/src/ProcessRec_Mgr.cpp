   ...
   /*如果输入输出文件的格式一样，则从TXTFILE_FMT中获得字段对应关系 
	 ...
	 */		 
	    ...
	    conn.close();
  		...
  		cout<<e.what()<<endl;
  		theJSLog << "根据输入输出文件格式ID取字段对应关系 出错" << endi;
  		throw jsexcp::CException(0, "根据输入输出文件格式ID取字段对应关系 出错", __FILE__, __LINE__);
  		conn.close();
  		return -1;
     }           
	...
	if (dbConnect(conn))
	 {
	    ...
	    conn.close();
  		...
  		cout<<e.what()<<endl;
  		theJSLog << "根据输出格式ID取格式类型 出错" << endi;
  		throw jsexcp::CException(0, "根据输出格式ID取格式类型 出错", __FILE__, __LINE__);
  		conn.close();
  		return -1;
     } 
	return ( 0 );
  ...
  DBConnection conn;//数据库连接
	...
	if (dbConnect(conn))
	 ...
	 conn.close();
		...
		cout<<e.what()<<endl;
		theJSLog << "取分拣规则 出错" << endi;
		throw jsexcp::CException(0, "取分拣规则 出错", __FILE__, __LINE__);
		conn.close();
		return false;
     } 
	...
	if (dbConnect(conn))
	 {
	    ...
	    conn.close();
   		...
   		cout<<e.what()<<endl;
   		theJSLog << "检测该过程共多少种处理方法 出错" << endi;
   		throw jsexcp::CException(0, "检测该过程共多少种处理方法 出错", __FILE__, __LINE__);
   		conn.close();
   		return false;
     }      
