  ...
  try{		            
	if (dbConnect(conn))
	  {		
		    ...
		    for (i=0; i<iCount; i++) 
            {
               stmt>>m_ChargeType[i].iChargeType>>m_ChargeType[i].iColindexFeeA>>
                     ...
                     m_ChargeType[i].iColindexRateAddB>>m_ChargeType[i].iColindexRepnoCdrduration>>
                      ...
                      m_ChargeType[i].iColindexRateFlag;
             } //end for
		...
		cout<<e.what()<<endl;
		theJSLog << "获取批价数据出错" << endi;
		throw jsexcp::CException(0, "获取数据出错", __FILE__, __LINE__);
		conn.close();
		return false;
     } 
...
bool CChargeType::getRecord(SChargeType &CT)
{
  ...
  if (ifind==1) 
  {
    ...
    CT.iColindexRateFlag = cur_ChargeType->iColindexRateFlag; 
    return true;
  }
  else 
  {
