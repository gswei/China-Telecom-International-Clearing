...
*/
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SMS_SETTLE_%s(KMT_FLAG,SOURCE_ID,PROVINCE_ID,PERCENT,PERORG,PERBASE,RMB_SDR_RATE) " 
					"select 1,'IISM',province_id,0,sum(city_settle_fee),%lf,RMB_SDR_RATE from D_IICC_SHARE_RESULT_%s where province_id <> 199 and home_country_id  in('9','37') group by province_id,RMB_SDR_RATE order by province_id ",mrate_cycle,total_base,mrate_cycle);
		cout<<"sql = "<<sql<<endl;
...
*/
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SMS_SETTLE_%s(KMT_FLAG,SOURCE_ID,PROVINCE_ID,PERCENT,PERORG,PERBASE,RMB_SDR_RATE) " 
					"select 0,'IISM',province_id,0,sum(city_settle_fee),%lf,RMB_SDR_RATE from D_IICC_SHARE_RESULT_%s where province_id <> 199 and home_country_id not in('9','37') group by province_id,RMB_SDR_RATE order by province_id ",mrate_cycle,total_base,mrate_cycle);
		cout<<"sql = "<<sql<<endl;
		stmt.setSQLString(sql);
		stmt.execute();
		...
		//更新
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_SMS_SETTLE_%s set PERCENT=PERORG/PERBASE,PERORG=PERORG*%lf/PERBASE,PERBASE=%lf where SOURCE_ID='IISM' ",mrate_cycle,sms_total_fee,sms_total_fee);
		stmt.setSQLString(sql);
		stmt.execute();
...
*/		
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SMS_SETTLE_%s(KMT_FLAG,SOURCE_ID,PROVINCE_ID,PERCENT,PERORG,PERBASE,RMB_SDR_RATE) " 
					"select 1,'IOSM',province_id,0,sum(city_settle_fee),%lf,RMB_SDR_RATE from D_IOCC_SHARE_RESULT_%s where province_id <> 199 and visit_country_id  in('9','37') group by province_id,RMB_SDR_RATE order by province_id ",mrate_cycle,total_base,mrate_cycle);
		stmt.setSQLString(sql);
		stmt.execute();
...
*/		
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SMS_SETTLE_%s(KMT_FLAG,SOURCE_ID,PROVINCE_ID,PERCENT,PERORG,PERBASE,RMB_SDR_RATE) " 
					"select 0,'IOSM',province_id,0,sum(city_settle_fee),%lf,RMB_SDR_RATE from D_IOCC_SHARE_RESULT_%s where province_id <> 199 and visit_country_id not in('9','37') group by province_id,RMB_SDR_RATE order by province_id ",mrate_cycle,total_base,mrate_cycle);
		stmt.setSQLString(sql);
		stmt.execute();
		
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_SMS_SETTLE_%s set PERCENT=PERORG/PERBASE,PERORG=PERORG*%lf/PERBASE,PERBASE=%lf where SOURCE_ID='IOSM'",mrate_cycle,sms_total_fee,sms_total_fee);
		stmt.setSQLString(sql);
		stmt.execute();
...
*/		
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into D_SMS_SETTLE_%s(KMT_FLAG,SOURCE_ID,PROVINCE_ID,PERCENT,PERORG,PERBASE,RMB_SDR_RATE) " 
					"select 2,'IOCM',province_id,0,sum(city_settle_fee),%lf,RMB_SDR_RATE from D_IOCM_SHARE_RESULT_%s group by province_id,RMB_SDR_RATE order by province_id ",mrate_cycle,total_base,mrate_cycle);
		stmt.setSQLString(sql);
		stmt.execute();

		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_SMS_SETTLE_%s set PERCENT=PERORG/PERBASE,PERORG=PERORG*%lf/PERBASE,PERBASE=%lf where SOURCE_ID='IOCM' ",mrate_cycle,sms_total_fee,sms_total_fee);
		stmt.setSQLString(sql);
		stmt.execute();
		...
		sprintf(sql,"update D_SMS_SETTLE_%s set PERORG_RMB=round(PERBASE*PERCENT*RMB_SDR_RATE/1000,2) ",mrate_cycle);
		stmt.setSQLString(sql);
		stmt.execute();
