...
int main(int argc,char** argv)
{
	...
	cout<<"*     last updaye time :  2013-06-06 by  hed	 "<<endl;
	cout<<"********************************************* "<<endl;
	...
	{		
		...
		sprintf(sql, "insert into D_SETTLE_LOG(source_id,deal_time,billmonth,deal_flag,job_id) values('%s',to_char(SYSDATE,'yyyymmddhh24miss'),'%s','%s','%s')",
	                  "IISM",mrate_cycle,"W","31");
		stmt.setSQLString(sql);
		stmt.execute();
		memset(sql,0,sizeof(sql));
		sprintf(sql, "insert into D_SETTLE_LOG(source_id,deal_time,billmonth,deal_flag,job_id) values('%s',to_char(SYSDATE,'yyyymmddhh24miss'),'%s','%s','%s')",
	                  "IOSM",mrate_cycle,"W","32");
		stmt.setSQLString(sql);
		stmt.execute();
...
*/		
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql, "update D_SETTLE_LOG set deal_flag = 'Y' where deal_flag ='W' and job_id in('%s','%s')","31","32");
		stmt.setSQLString(sql);
		stmt.execute();
	 ...
	 {
		memset(sql,0,sizeof(sql));
		sprintf(sql, "update D_SETTLE_LOG set deal_flag = 'E' where deal_flag ='W' and job_id in('%s','%s')","31","32");
		stmt.setSQLString(sql);
		stmt.execute();
		stmt.close();
