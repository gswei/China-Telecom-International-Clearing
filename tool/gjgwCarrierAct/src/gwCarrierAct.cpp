...
{	
	...
	cout<<"gjgw_dataget -t YYYYMM　6  CTOG LTE结算数据统计接口文件\n" <<endl;
...
//全球虚拟总机数据提取 接口文件格式：文本文件。文件中的字段之间用“|”分隔。文件中一行作为一条统计记录，各条记录之间用换行符“\n”作为分隔
...
//CTOG LTE结算数据统计接口 接口文件格式：文本文件。文件中的字段之间用“,”逗号分隔。文件中一行作为一条统计记录，各条记录之间用换行符“\n”作为分隔
void CarrierAct::run6()
{
	if(!(dbConnect(conn)))
	 {
		memset(erro_msg,0,sizeof(erro_msg));
	...
	Statement stmt;
	//int column_count = 7;
	char outFPathName[512],outFPath[256];
	...
	try
	{
		...
		//删除该账期数据
		sprintf(sql,"delete from CTOG_LTE_SETTLE_DATA where ratecycle=%s ",ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();

		//提取每月天翼MIFI ：MIFI业务指CD*CHNXY*的文件
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into CTOG_LTE_SETTLE_DATA "
				"select a.ratecycle,b.sponsor_cd as sponsor_code,'10' as serv_type,b.ct_carrier_cd as file_nm,"
						"round(sum(nvl(a.basicfeesdr,0)+nvl(a.roamingfeesdr,0))/1000,2) as settle_fee_SDR,"
						" '1' as measure_type,sum(ceil(nvl(a.chargedataflow,0)/1024)) as value,"
						"b.carrier_ctry as Visited_Country "
				"from d_ixcg_base_result_%s a ,CTOG_CARRIER b "
				"where a.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNXY' "
						"and a.flag10 like 'CD%%CHNXY%%' " //and b.carrier_ctry in('香港','澳门') "
				"group by a.ratecycle,b.sponsor_cd,b.ct_carrier_cd,b.carrier_ctry "
				,ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();
		
		//CtoG业务 10万个Imsi的话务量统计
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into CTOG_LTE_SETTLE_DATA "
				"select a.ratecycle,b.sponsor_cd as sponsor_code,'12' as serv_type,b.ct_carrier_cd as file_nm,"
						"round(sum(nvl(a.basicfeesdr,0)+nvl(a.roamingfeesdr,0))/1000,2) as settle_fee_SDR,"
						"'2' as measure_type,sum(ceil(nvl(a.duration,0)/60)) as value,'0' as Visited_Country "
				"from d_iocg_base_result_%s a ,CTOG_CARRIER b "
				"where a.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNX1' "
						"and a.flag10 like 'CD%%CHNX1%%' "
				"group by a.ratecycle,b.sponsor_cd,b.ct_carrier_cd,b.carrier_ctry"
				,ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into CTOG_LTE_SETTLE_DATA "
				"select a.ratecycle,b.sponsor_cd as sponsor_code,'11' as serv_type,b.ct_carrier_cd as file_nm,"
						"round(sum(nvl(a.basicfeesdr,0)+nvl(a.roamingfeesdr,0))/1000,2) as settle_fee_SDR,"
						"'0' as measure_type,0 as value,'0' as Visited_Country "
				"from d_iocg_base_result_%s a ,CTOG_CARRIER b "
				"where a.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNXT' "
						"and a.flag10 like 'CD%%CHNXT%%' "
				"group by a.ratecycle,b.sponsor_cd,b.ct_carrier_cd,b.carrier_ctry"
				,ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"insert into CTOG_LTE_SETTLE_DATA "
				"select ratecycle,sponsor_cd as sponsor_code,'30' as serv_type,ct_carrier_cd as file_nm, "
						"sum(nvl(fee_sdr,0)) as settle_fee_SDR,'0' as measure_type,0 as value,'0' as Visited_Country "
				"from( "
					"select a.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a.basicfeesdr,0)+nvl(a.roamingfeesdr,0))/1000,2) as fee_sdr "
					"from d_ixcg_base_result_%s a ,CTOG_CARRIER b "
					"where a.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNTE' "
						"and a.flag10 like 'CD%%CHNTE%%' "
					"group by a.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
				"union all "
					"select a1.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a1.basicfeesdr,0)+nvl(a1.roamingfeesdr,0))/1000,2) as fee_sdr "
					"from d_iocg_base_result_%s a1 ,CTOG_CARRIER b "
					"where a1.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNTE' "
						"and a1.flag10 like 'CD%%CHNTE%%' "
					"group by a1.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
				"union all "
					"select a2.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a2.totlefeesdr,0))/1000,2) as fee_sdr "
					"from d_imcg_base_result_%s a2 ,CTOG_CARRIER b "
					"where a2.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHNTE' "
						"and a2.flag10 like 'CD%%CHNTE%%' "
					"group by a2.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
			")group by ratecycle,sponsor_cd,ct_carrier_cd "
				,ratecycle,ratecycle,ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();
		...
		sprintf(sql,"insert into CTOG_LTE_SETTLE_DATA "
				...
				"from( "
					"select a.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a.basicfeesdr,0)+nvl(a.roamingfeesdr,0))/1000,2) as fee_sdr "
					"from d_ixcg_base_result_%s a ,CTOG_CARRIER b "
					"where a.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHN4G' "
						"and a.flag10 like 'CD%%CHN4G%%' "
					"group by a.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
				"union all "
					"select a1.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a1.basicfeesdr,0)+nvl(a1.roamingfeesdr,0))/1000,2) as fee_sdr "
					"from d_iocg_base_result_%s a1 ,CTOG_CARRIER b "
					"where a1.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHN4G' "
						"and a1.flag10 like 'CD%%CHN4G%%' "
					"group by a1.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
				"union all "
					"select a2.ratecycle,b.sponsor_cd,b.ct_carrier_cd, "
						"round(sum(nvl(a2.totlefeesdr,0))/1000,2) as fee_sdr "
					"from d_imcg_base_result_%s a2 ,CTOG_CARRIER b "
					"where a2.homecarrier=b.carrier_cd and b.ct_carrier_cd='CHN4G' "
						"and a2.flag10 like 'CD%%CHN4G%%' "
					"group by a2.ratecycle,b.sponsor_cd,b.ct_carrier_cd "
			")group by ratecycle,sponsor_cd,ct_carrier_cd "
				,ratecycle,ratecycle,ratecycle);
		stmt.setSQLString(sql);
		...
		memset(outFPathName,0,sizeof(outFPathName));
		sprintf(outFPathName,"%sSettleData_%s.txt",outFPath,ratecycle);
    ...
    memset(sql,0,sizeof(sql));
		sprintf(sql,"select ratecycle, sponsor_code, serv_type, file_nm, "
					"decode(substr(to_char(nvl(settle_fee_SDR,0)),1,1),'.','0','')||to_char(nvl(settle_fee_SDR,0)), "
					"measure_type, "
					"nvl(value,0), "
				  "Visited_Country "
				"from CTOG_LTE_SETTLE_DATA "
				"where ratecycle=%s "
				"order by sponsor_code,serv_type,Visited_Country "
				,ratecycle);
		stmt.setSQLString(sql);
		stmt.execute();
