...
{	
	cout<<"命令格式: \n"<<endl;
	cout<<"settleBalance -t 账期 -n 年份 -j 季度 \n"<<endl;
}
...
void Balance::run()
{
	...
	try
    {
		...
		{
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"国际固网短信调整数据导入表 D_GWDX_ADJUST_INPUT_DATA 没有账期[%s]的数据",ratecycle);
			//theJSLog.writeLog(LOG_CODE_DB_CONNECT_ERR,erro_msg);		//连接数据库失败
			theJSLog<<erro_msg<<endw;
			return ;
		}
...
*/
		...
		if(cnt == 0)
		{
			stmt.close();
			memset(erro_msg,0,sizeof(erro_msg));
			sprintf(erro_msg,"国际固网语音调整数据导入表 D_GWYY_ADJUST_INPUT_DATA 没有账期[%s]的数据",ratecycle);
			//theJSLog.writeLog(LOG_CODE_DB_CONNECT_ERR,erro_msg);		//连接数据库失败
			theJSLog<<erro_msg<<endw;
			return ;
		}
