...
void Balance::run()
{
	...
	try
    {
		...
		//if(strncmp(ratecycle+4,"04",2) == 0)  2015-03-01 特殊情况只针对2014-04
		if(strncmp(ratecycle,"201404",6) == 0)
		{
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_GWDX_ADJ_OUTPUT_DATA_%s c set c.IN_ADJ_FEE = round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),c.OUT_ADJ_FEE = round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),"
					"c.IN_ADJ_USD_FEE=round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),c.OUT_ADJ_USD_FEE=round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2) "
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_GWDX_ADJ_OUTPUT_DATA_%s a set a.prov_percent=a.prov_percent+(%lf),a.in_adj_fee=a.in_adj_fee+round((%lf),2),a.out_adj_fee=a.out_adj_fee+round((%lf),2),"
					"a.in_adj_usd_fee=a.in_adj_usd_fee+round((%lf),2),a.out_adj_usd_fee=a.out_adj_usd_fee+round((%lf),2) "
...
*/
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_GWYY_ADJ_OUTPUT_DATA_%s c set c.IN_ADJ_FEE = round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),c.OUT_ADJ_FEE = round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),"
					"c.IN_ADJ_USD_FEE=round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2),c.OUT_ADJ_USD_FEE=round(c.UNIT_BILL_MIN/c.TOTAL_BILL_MIN*%lf,2) "
					"where c.RATECYCLE = %s ",ratecycle,in_adj_fee,out_adj_fee,in_adj_usd_fee,out_adj_usd_fee,ratecycle);
		theJSLog<<"计算各省份单位时长:"<<sql<<endi;
		stmt.setSQLString(sql);
		...
		memset(sql,0,sizeof(sql));
		sprintf(sql,"update D_GWYY_ADJ_OUTPUT_DATA_%s a set a.prov_percent=a.prov_percent+(%lf),a.in_adj_fee=a.in_adj_fee+round((%lf),2),a.out_adj_fee=a.out_adj_fee+round((%lf),2),"
					"a.in_adj_usd_fee=a.in_adj_usd_fee+round((%lf),2),a.out_adj_usd_fee=a.out_adj_usd_fee+round((%lf),2) "
					"where a.RATECYCLE = %s and a.kmt_flag = 1 and a.PROVINCE = 20 "
					,ratecycle,(percent_tmp-percent),(in_adj_fee-in_adj_fee_tmp),(out_adj_fee-out_adj_fee_tmp),(in_adj_usd_fee-in_adj_usd_fee_tmp),(out_adj_usd_fee-out_adj_usd_fee_tmp),ratecycle);
