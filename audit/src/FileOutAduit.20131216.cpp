...
{	
	...
	try
	{
		...
		if(mdrParam.drStatus == 1)		//主备系统
		{
			...
			strcpy(before_date,mdrParam.m_SerialString);	
		...
		{
			sprintf(mdrParam.m_SerialString,"%s%s",mdrParam.m_SerialString,before_date);
			ret = drVarGetSet(mdrParam.m_SerialString);
			if(ret)
			{
		...
		for(map< string,AduitEnv >::const_iterator it3=fileNameMap.begin() ;it3!=fileNameMap.end();++it3)
		{
			...
			sprintf(mdrParam.m_AuditMsg,"%s%s,%d,%c,",mdrParam.m_AuditMsg,m_szFileName,arrayFile.size(),(it3->second).null_out_flag);

			if(arrayFile.size() == 0)
			{		
				 if((it3->second).null_out_flag == 'N')	
				 {
					 sprintf(mdrParam.m_AuditMsg,"%s|",mdrParam.m_AuditMsg);
					 continue ;	
				 }
			}
			...
			sprintf(mdrParam.m_AuditMsg,"%s%ld|",mdrParam.m_AuditMsg,fileInfo.st_size);	
		}
...
int main(int argc,char** argv)
{
	...
	cout<<"*     last updaye time :  2013-12-02 by  hed	 "<<endl;
	cout<<"********************************************* "<<endl;
