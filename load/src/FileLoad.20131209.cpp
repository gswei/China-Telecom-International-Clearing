...
int FileLoad::onBeforeTask()
{
	...
	{	
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrParam.m_SerialString,";",data,false,false);  //发送的字符串数据源ID,文件名,
	...
	else								//主系统,非容灾系统
	{
		...
		while(1)		
		{
				...
				memset(mdrParam.m_SerialString,0,sizeof(mdrParam.m_SerialString));
				sprintf(mdrParam.m_SerialString,"%s;%s;%d",m_szSourceID,m_szFileName,petri_status);
				ret = drVarGetSet(mdrParam.m_SerialString);
				if(ret)
				{
					theJSLog<<"同步失败...."<<endw;
...
*/
			...
			sprintf(mdrParam.m_AuditMsg,"%s;%s;%d;%d",m_szSourceID,m_szFileName,record_num,mConfParam.maxRecord_num);
	/*
...
{  
	...
	else
	{
		...
		int ret = IsAuditSuccess(mdrParam.m_AuditMsg);					//仲裁弄到申请话单块完成后
		if(ret)
		{
			if(petri_status == DB_STATUS_ONLINE)
			{
