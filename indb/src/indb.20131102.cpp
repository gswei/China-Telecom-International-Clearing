...
int C_Indb::LoadSourceCfg()
{
	...
	{		
		...
		for(map<string,SOURCECFG>::const_iterator iter = m_SourceCfg.begin();iter !=m_SourceCfg.end(); ++iter)
		{
			...
			TabConf[index].init(iter2->second,m_szOutTypeId);
...
{	
	int ret = 0;	
	char dir[JS_MAX_FILEFULLPATH_LEN],inputFilePath[JS_MAX_FILEFULLPATH_LEN],filter[50],szFiletypeIn[10];
	...
	{
		theJSLog<<"数据库状态切换... "<<petri_status<<"->"<<db_status<<endi;
		int cmd_sn = 0;
	...
	{	
			...
			//获取同步变量
			vector<string> data;		
			splitString(m_SerialString,";",data,false,false);  //发送的字符串数据源ID,文件名
	...
	else
	{
		...
		{		
				...
				//发送同步信息
				memset(m_SerialString,0,sizeof(m_SerialString));
				sprintf(m_SerialString,"%s;%s;%d",m_szSourceID,m_szFileName,petri_status);
				ret = drVarGetSet(m_SerialString);
				if(ret)
				{
					theJSLog<<"同步失败...."<<endw;
...
{	
	...
	}catch (jsexcp::CException e)
	{
		...
		char tmp[512];
		sprintf(m_AuditMsg,"%s;%s;%c;%d",m_szSourceID,m_szFileName,state,record_num);
		theJSLog<<"wait dr_audit() ..."<<endi;
		if(!IsAuditSuccess(m_AuditMsg))
		{
			tab.RollBack();
			theJSLog<<"移动文件到失败目录:"<<erro_path<<endi;
...
int main(int argc,char** argv)
{
	...
	cout<<"*     last updaye time :  2013-11-02 by  hed	  "<<endl;
	cout<<"********************************************** "<<endl;
