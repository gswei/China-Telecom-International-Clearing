...
{	
 ...
 while(1)	
 {
	...
	{	
			...
			//获取同步变量
			vector<string> data;		
			splitString(mdrParam.m_SerialString,";",data,false,false);  //发送的字符串数据源ID,文件名,sqlFile文件名
...
{	
	...
	}catch (jsexcp::CException e)
	{
		...
		char tmp[512];
		sprintf(mdrParam.m_AuditMsg,"%s;%s;%c;%d",m_szSourceID,m_szFileName,state,record_num);
...
int main(int argc,char** argv)
{
	...
	cout<<"*     last updaye time :  2013-11-14 by  hed	  "<<endl;
	cout<<"********************************************** "<<endl;
