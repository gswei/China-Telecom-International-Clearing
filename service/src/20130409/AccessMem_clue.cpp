...
int C_AccessMem::init(char* serverId, char* iniPath)
{
	...
	CBindSQL sql(DBConn);
	sprintf(szSql, "select count(a.rowid) from c_commem_table_def a, c_commem_control c where \
		a.mem_define_id=c.shmkey_id and c.server_id='%s' and c.valid_flag = 'Y'", serverId);
	sql.Open(szSql, SELECT_QUERY);
	if(!(sql>>m_iTableNum))
	{
		cout<<"没找到num"<<endl;
		sql.Close();
	...
	if(tableInfo == NULL)
	{
		...
		throw CException(PREDEAL_ERR_NOT_ENOUGH_MEMORY, ErrorMsg, __FILE__, __LINE__);
	}
	sql.Open(szSql, SELECT_QUERY);
	sprintf(szSql, "select trim(a.update_table_name), to_number(c.version_id), count(b.column_id) \
		from c_commem_table_def a, c_commem_column_def b, c_commem_control c where \
		a.mem_define_id=b.mem_define_id and a.mem_define_id=c.shmkey_id and c.server_id='%s' \
		and c.valid_flag = 'Y' group by a.update_table_name, c.version_id ", serverId);
	sql.Open(szSql, SELECT_QUERY);
	//while(sql>>memName>>versionId>>columnNum)
	for(iNum =0; iNum < m_iTableNum; iNum++)
	{
		sql>>tableInfo[iNum].tableName>>tableInfo[iNum].tableVersion>>tableInfo[iNum].colNum;
		theLog<<"连接共享内存 "<<serverId<<"==="<<tableInfo[iNum].tableName<<"==="<<tableInfo[iNum].tableVersion<<"==="<<iniPath<<endi;
		tableInfo[iNum].table = new CommonMemClient(serverId, tableInfo[iNum].tableName, tableInfo[iNum].tableVersion, iniPath);
		//tableInfo[iNum].table->init(serverId, tableInfo[iNum].tableName, tableInfo[iNum].tableVersion, iniPath);
		/*
		if(m_iTableNum >= MAX_TABLE_NUM-1)
		{
			sprintf( ErrorMsg, "共享内存中的表数目超过规定值 %d", MAX_TABLE_NUM);
			throw CException(0, ErrorMsg, __FILE__, __LINE__);
		}
		table[m_iTableNum] = 0;
		theLog<<"连接共享内存 "<<serverId<<"==="<<memName<<"==="<<versionId<<"==="<<iniPath<<endi;
		table[m_iTableNum] = new CommonMemClient(serverId, memName, versionId, iniPath);
		tableMap.insert(map<string, CommonMemClient*>::value_type(memName, table[m_iTableNum]));
		tableInfo.insert(map<string, int>::value_type(memName, columnNum));
		m_iTableNum++;
		*/
	}
	sql.Close();
	...
	int searchType;
	tableIndex tblIndex;
	sprintf(szSql, "select trim(b.update_table_name), to_number(a.index_id), a.index_type from c_commem_index_def a, \
		c_commem_table_def b , c_commem_mem_version c where a.mem_define_id = b.mem_define_id and \
		b.mem_define_id=c.mem_define_id and c.server_id='%s'", serverId);
cout<<szSql<<endl;
	sql.Open(szSql, SELECT_QUERY);
	while(sql>>tblIndex.szName>>tblIndex.iIndex>>type)
	{
...
cout<<"insert:"<<tblIndex.szName<<";"<<tblIndex.iIndex<<";"<<searchType<<endl;
	}
	sql.Close();
...
{
	cout<<"Search table "<<tableOffset<<" with index = "<<iSearchIndex<<endl;
	//timerclear(&start);
	//timerclear(&finish);
...
int C_AccessMem::getSearchType(char* tableName, int iTableIndex)
{
	...
	cout<<tbl.szName<<"|"<<tbl.iIndex<<endl;
...
{
	//theLog<<"查询共享内存"<<searchNum<<"次，用时"<<timePart1<<";"<<timePart2<<endi;
	searchNum=0;
	//timePart1=0;
	//timePart2=0;
}
