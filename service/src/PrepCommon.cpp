...
int C_DealCalculator::check()
{
	...
	//cout << "vRecordPacket.size() = " << vRecordPacket.size()<<endl;
...
int C_DealCalculator::updateTable(const char* tableName, struct SFileStruct &filestruct, int proc_index)
{
	...
	theJSLog<<"---- "<<filestruct.szRealFileName<<" dealed "<<m_TotalNum<<" records"<<endi;
	...
	if (dbConnect(conn))
	 {
	    ...
	    if(vRecordPacket.size() == 1)
	     {
			...
			theJSLog<<"---- SERV_CAT "<<vRecordPacket[0].szSerCatId<<" with "<<vRecordPacket[0].iRightNum<<" items into mainflow, "
			<<vRecordPacket[0].iLackNum<<" lack_info items , "<<vRecordPacket[0].iErrorNum<<" abnormal items and "<<vRecordPacket[0].iPickNum
			<<" pick items!"<<endi;
		    theJSLog<<"---- and "<<vRecordPacket[0].iOtherNum<<" items in the pick path!"<<endi;
	    ...
	    else
	   {
		...
		for(int i=0; i<vRecordPacket.size(); i++)
		{
			...
			theJSLog<<"---- SERV_CAT "<<vRecordPacket[i].szSerCatId<<" with "<<vRecordPacket[i].iRightNum<<" items into mainflow, "
				<<vRecordPacket[i].iLackNum<<" lack_info items , "<<vRecordPacket[i].iErrorNum<<" abnormal items and "<<vRecordPacket[i].iPickNum
				<<" pick items!"<<endi;
			theJSLog<<"---- and "<<vRecordPacket[i].iOtherNum<<" items in the pick path!"<<endi;
		}
	  }
	 ...
	 conn.close();
		...
		cout<<e.what()<<endl;
		conn.close();
		throw jsexcp::CException(0, "更新调度表失败", __FILE__, __LINE__);
		return -1;
 } 	 
	theJSLog<<"Update in_schedule set to _Y_ successfully!"<<endi;
	return 0;
}
...
int C_DealCalculator::updateTable(const char* tableName, char *filename, char *dealstarttime, char *sourceID,int proc_index )
{
	  ...
	  if (dbConnect(conn))
	  {
	     ...
	     if(vRecordPacket.size() == 1)
	      {	
	         ...
	         sprintf(szSqlStr, "insert into %s(source_id,serv_cat_id,filename,deal_flag,dealstarttime,dealendtime,input_count, \
		      mainflow_count,error_count,lackinfo_count,pick_count,other_count,validflag,partid,proc_index) values\
		      ('%s', '%s', '%s', 'Y', '%s', '%s', %d, %d, %d, %d, %d, %d, 'Y' , %d, %d)", tableName, sourceID, vRecordPacket[0].szSerCatId,\
		      filename , dealstarttime, CurrentTime, vRecordPacket[0].iTotalNum, vRecordPacket[0].iRightNum ,\
		      vRecordPacket[0].iErrorNum, vRecordPacket[0].iLackNum, vRecordPacket[0].iPickNum, 
			...
			/*sql = "insert into :v1(source_id,serv_cat_id,filename,deal_flag,dealstarttime,dealendtime,input_count, \
		      ...
		      //cout << "szSqlStr = "<<szSqlStr <<endl;
		      // 20131023 add petri网状态
	   ...
	   //     rtinfo.getDBSysMode(petri_status);		//获取状态
		...
		//	 stmt.setSQLString(szSqlStr);
		//	 if(petri_status==303)
		//	{
		//	   stmt.execute();
		//	   conn.commit();	
	//	   memset(outSqlStr,0,sizeof(outSqlStr));
		//    } else
		//    {
		     // 将sql 语句传到外面
		     strcpy(outSqlStr,szSqlStr);
		//    }
            ...
            theJSLog<<"filename:"<<filename<<"---- SERV_CAT "<<vRecordPacket[0].szSerCatId<<" with "<<vRecordPacket[0].iRightNum<<" items into mainflow, "
			    <<vRecordPacket[0].iLackNum<<" lack_info items , "<<vRecordPacket[0].iErrorNum<<" abnormal items and "<<vRecordPacket[0].iPickNum
			    <<" pick items!"<<endi;
		   theJSLog<<"---- and "<<vRecordPacket[0].iOtherNum<<" items in the pick path!"<<endi;	
	}
	else
	{
		...
		cout<<e.what()<<endl;
		conn.close();
		throw jsexcp::CException(0, "插入调度表失败", __FILE__, __LINE__);
    }
	theJSLog<<"insert in_schedule set to _Y_ successfully!"<<endi;
	return 0;
}
...
char * C_DealCalculator::getSqlstr()
{
	return outSqlStr;
}
