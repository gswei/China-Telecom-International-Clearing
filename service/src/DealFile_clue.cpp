...
//	_outrcd.Init(Param.szOutputFiletypeId, Param.szOutrcdType[0]);
	...
	theJSLog<<"初始化文件使用时间："<<timeMicros<<endi;
	gettimeofday(&start, NULL);
	*/
	...
	try
	{
		while(1)
		{
			...
			try {
				...
				if( strcmp(Param.szIsFmtFirst,"Y")==0 )
				{
					...
					if( _infile.readRec(szBuff, MAX_LINE_LENGTH) != READ_AT_END )
					{
						...
						//theJSLog<<"输入第"<<iTotalNum+1<<"行："<<pps->getString()<<endd;
					}
					else
					{
						if(iTotalNum == 0)
			...
			//gettimeofday(&finish, NULL);
//			theJSLog<<"初始化话单信息用时："<<timeMicros<<endi;
...
//				{
//					theJSLog<<"LINE："<<iTotalNum<<"<<当前执行的插件/表达式ID："<<ret->getRelatedActionId()<<"   类型：" <<ret->getRelatedActionType()<<"   用时： "<<ret->getUsedTimeMicros()<<endd;
//				}
				theJSLog<<"LINE："<<iTotalNum<<"<<当前执行的插件/表达式ID："<<ret->getRelatedActionId()<<"   类型：" <<ret->getRelatedActionType()<<endi;
				if( ret->getRelatedActionType() != "PLUGIN-EXECUTOR" )
					continue;
				Result = pRcd->getAnaType();
				cout<<"Result = "<<Result<<endl;;
				theJSLog<<res->m_outRcd.Get_record()<<endi;
				if( Result == eNormal)
				{
					pps->copyRes(*res);
...
/*			gettimeofday(&finish, NULL);
			timeMicros = (finish.tv_sec - start.tv_sec) * (long) 1000 * (long) 1000 + (finish.tv_usec - start.tv_usec);
			theJSLog<<"用插件分析第"<<iTotalNum<<"条话单用时："<<timeMicros<<endi;
			*/
			//gettimeofday(&start, NULL);
		...
		theJSLog << "##################### file process in " << timeAll << "(micros)" << endi;
		theJSLog << "######################## timePart1:" << timePart1 << endi;
		theJSLog << "######################## timePart2:" << timePart2 << endi;
		theJSLog << "######################## timePart3:" << timePart3 << endi;
		theJSLog << "######################## timePart4:" << timePart4 << endi;
		*/
