...
void CClassify::endFile()
{
	for(CLASSIFY_RULE::iterator it=rule.begin(); it != rule.end(); it++)
	{
        ...
        vector<string> filenames =  it->second.filename;
		vector<string>::iterator iter = filenames.begin();
        for(; iter != filenames.end(); iter++)
		{
			//cout << "realname = " << *iter << endl;			
		}			
	}	
	
}

void CClassify::getAllRule(int& tmprule,vector<string>& rule_filename)
{
	for(CLASSIFY_RULE::iterator it=rule.begin(); it != rule.end(); it++)
	{
	    it->second.getFilename();
	    tmprule = it->first;
	    //cout << "tmprule = " << tmprule<<endl;
		vector<string> filenames =  it->second.filename;
		vector<string>::iterator iter = filenames.begin();
        for(; iter != filenames.end(); iter++)
		{
			//cout << "realname = " << *iter << endl;	
			rule_filename.push_back(*iter);
		}
	}
}
...
int CClassifyRule::dealRecord(CFmt_Change& inrcd)
{
	...
	else
	{
		...
		//filename.push_back(pick_info.szRealName);
		if(!strcmp(m_OutputType,"I"))
		{
...
{
	theJSLog<<"classify("<<m_PickType<<") commit "<<endi;
	char errmsg[512];
	//CBindSQL ds(DBConn);
    try{			
    if (dbConnect(conn))
	...
	else
	{
		for(int idx=0; idx<m_OutInfo.size(); idx++)
		{
			if(m_OutInfo[idx].iPickNum > 0)
			{
				...
				theJSLog<<"classify("<<m_PickType<<") rename "<<m_OutInfo[idx].szTmpName<<" to "<<m_OutInfo[idx].szRealName<<endi;
				//分拣记录插入D_FORMAT_STAT_LOG表
				if(m_LogFlag[0] == 'Y')
				{
				    sql = "insert into D_FORMAT_STAT_LOG(source_id, filename, pick_target, pick_path, classify_type_id, inserttime) \
			...
			else
			{
				...
				theJSLog<<"classify("<<m_PickType<<") unlink "<<m_OutInfo[idx].szTmpName<<" : "<<m_OutInfo[idx].iPickNum<<endi;
			}
		 }
		m_OutInfo.clear();
	   }
...
void CClassifyRule::getFilename()
{
   sPickInfo pick_info;
   VEC_PINFO::iterator iter = m_OutInfo.begin();
   for(; iter != m_OutInfo.end(); iter++)
		{
			filename.push_back(iter->szRealName);		
		}
}
