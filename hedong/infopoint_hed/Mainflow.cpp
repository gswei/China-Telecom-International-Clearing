#include "Mainflow.h"
//2014-03-18  增加对表统计功能,增加对5分钟定时的处理(addmin改为+5,可能会导致程序暂停下次启动会重传上个结束的开始时间点)
//2014-04-01  解决程序不能写错误日志的功能,捕获异常情况CF_CError
//2015-01-12  解决CheckLock 日,月时重复信息点生成的问题15改为5
//2015-01-16  解决在时间段生成信息点按日生成时统计到上一天的数据问题
...
void main(int argc, char* argv[])
{
	...
	cout<<"*           created time : 2015-01-16 by  hedong	* "<<endl;
	cout<<"********************************************** "<<endl;
...
*/
		if(kk<5)				//2014-03-24 规则调整
		{
			strcpy(szTmpDt,"00");
		}
		else if(kk<55)
		{
			sprintf(szTmpDt,"%02d",kk/5*5);
		}
		else
		{
			strcpy(szTmpDt,"55");
	...
	if(wrlog_setEnvPath(szEnvFile)!=0)//设置环境变量文件，读取错误日志路径;
  {
	...
	try
	{
		...
		{	
			...
			sprintf(szLogStr,"%d|%s",ex.get_appErrorCode(),ex.get_errMessage());
			getCurTime(sz_errtime);
			wrlog("infopt","1","",'E','H',1001,002,sz_errtime,szLogStr,__FILE__,__LINE__);
			exit(1);
		}
		catch (...)
		{			
	...
	if (szProMode[0]=='T')
		{
		   ...
		   if (0!=infoDeal.Init(szInfoDetect,szService))
		   {
			   ...
			   getCurTime(sz_errtime);
               wrlog("infopt","1","",'E','H',1002,002,sz_errtime,szLogStr,__FILE__,__LINE__);			
			   exit(1);
		   }
	     ...
	     {
	   	   setLogPath(szLog_Dir, "infoMgr");	   	   
	   	   addmin(szTempStartTime,5,szTmpDt);
			   ...
			   for(i=0;i<g_sInfo.size(); i++)
			   {
		          ...
		          try
		          {
		      	    ...
		      	    if (infoDeal.GetBatch(g_sInfo[i].szInfoPoint,g_sInfo[i].szTypeId,g_sInfo[i].szRate)==-1)
		      	    {
		      	      ...
		      	      getCurTime(sz_errtime);
                  wrlog("infopt","1","",'E','H',1003,002,sz_errtime,szLogStr,__FILE__,__LINE__);	
                  exit(0);
		      	    }		
		 ...
		 /* sprintf(szSql, "SELECT  max(ENDTIME),count(*) from  INFO_PUB_BATCH ");//需要修改,加上szService的条件,在语音ab机上，会出问题
		  ...
		  while(ds>>szTempStartTime>>iBatchCount)
		  {
		    // expTrace("Y", __FILE__, __LINE__, "szTempStartTime is %s ",szTempStartTime);
		     addmin(szTempStartTime,5,szTempStartTime);
		  }
		  ds.Close();
          //expTrace("Y", __FILE__, __LINE__, "szTempStartTime is %s ",szTempStartTime);
...
*/
	...
	{	
		...
		sprintf(szLogStr,"%d|%s",ex.get_appErrorCode(),ex.get_errMessage());
		getCurTime(sz_errtime);
		wrlog("infopt","1","",'E','H',1005,002,sz_errtime,szLogStr,__FILE__,__LINE__);
		exit(1);
	}
	catch (...)
	{			
...
int GetRunFlag(int &iRunFlag, char *serv)
{
  ...
  {	
	...
	sprintf(szLogStr,"%d|%s",ex.get_appErrorCode(),ex.get_errMessage());
	getCurTime(sztt);
	wrlog("infopt","1","",'E','H',1003,002,sztt,szLogStr,__FILE__,__LINE__);
  }
  catch(...)
  {  
		getCurTime(sztt);
    wrlog("infopt","1","",'E','H',1003,002,sztt,"ger run_flag err",__FILE__,__LINE__);	
...
int CheckLock(char* szChTime,int iChDelay,char *szChCurDatetime,char*sFrom,char *sTo)
{
  ...
  ///////////////////////////////记录在表格中的时间字段  
    	/*sprintf(szFrom,"%s%000000",strLastday.c_str());
  		...
  		return 0;*/  		
  	if((HH_COM==HH)&&((MM-MM_COM)<5 && (MM>=MM_COM)))
  	{
  	  	//sprintf(szFrom,"%s%000000",strLastday.c_str());
  		//sprintf(szTo,"%s%000000",strToday.c_str());
		//2015-01-16
		sprintf(szFrom,"%s000000",strToday.c_str());
  		sprintf(szTo,"%s235959",strToday.c_str());
  ...
  {
  	if(strcmp(strCom_DD.c_str(),strNowTime_DD.c_str())==0&&(HH_COM==HH)&&((MM-MM_COM)<5 && (MM>=MM_COM)))
  	{
